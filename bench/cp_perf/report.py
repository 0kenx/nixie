#!/usr/bin/env python3
"""Compare all paired CP cells, rejecting trace differences and missing seeds."""
import argparse
...
def read(sha):
    rows = {}
    for f in (a.root / sha / 'benchmark/runs/cp-scheduling').glob('*.json'):
...
assert base.keys() == treatment.keys() and len(base) == 28 * a.seeds, 'incomplete comparison'
...
print(f'{len(rows)} exact output matches; solver/certified SAT cells={14 * a.seeds}; callback cells={14 * a.seeds}; no lost cells')
if a.csv:
