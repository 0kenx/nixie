#!/usr/bin/env python3
"""Fixed three-arm experiment; see the heap definition optimization preregistration.
...
from run import ROOT, benchstore, digest, generate, oracle, perf_count, summarize, FAMILIES

SUITE = "heap-definition-specialization-v1"
BASELINE_SHA = "097ca4ad4773f27acd37927afbf567b616b38acd"
ARMS = ("baseline", "identity", "specialized")
SEEDS = list(range(10)) + [101]
WORKER = Path(__file__).resolve().with_name("run.py")
        ...
        for seed in SEEDS:
            ...
            for arm,phase in ordered:
                ...
                flags = dict(arm=arm,phase=phase,cpu=args.cpu,cap_s=20,pythonhashseed=0,nixie_environment="cleared",pmu="instructions:u",harness_sha256=digest(__file__),
                             ...
                             definition_simplification=arm == "specialized",baseline_sha=BASELINE_SHA,
                ...
                rec = dict(schema=benchstore.SCHEMA,created_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),suite=SUITE,
                    ...
                    arm=dict(role="treatment" if arm == "specialized" else "null" if arm == "identity" else "baseline"))
                ...
                if result_file.exists():
                    ...
                    env["HEAP_PERF_UNFOLDED"] = "1" if arm == "identity" else "0"
                    ...
                    raw.with_suffix(".command.json").write_text(json.dumps(dict(command=command,
                        environment={k:env[k] for k in ("PYTHONHASHSEED", "HEAP_PERF_UNFOLDED")})))
