...
//! scripts (via [`crate::generator`]), run them through both Nixie and Z3
//! (via [`crate::nixie_runner`] / [`crate::z3_runner`]), and compare
...
use crate::nixie_runner::run_nixie;
...
pub fn save_repro(logic: Logic, seed: u64, script: &str) -> Result<PathBuf> {
    let dir = std::env::temp_dir().join("nixie_difftest_repro");
...
fn run_case_with_script(logic: Logic, seed: u64, script: String) -> Result<DiffCaseOutcome> {
    let scratch_dir = std::env::temp_dir().join("nixie_difftest_input");
    ...
    let nixie_result = run_nixie(&file_path).context("running Nixie on generated case")?;
...
pub fn format_mismatch_report(mismatches: &[DiffCaseOutcome]) -> String {
    let mut msg = format!("{} Nixie/Z3 verdict mismatch(es) found:\n", mismatches.len());
    for m in mismatches {
        ...
        msg.push_str(&format!(
            "  [{}] seed={} nixie={:?} z3={:?} repro={}\n",
            m.logic, m.seed, m.nixie_result, m.z3_result, repro
        ));
    }
    msg
}
...
mod tests {
    ...
    fn run_cases_is_deterministic_in_case_count() {
        ...
        // run both solvers; run_nixie always works (pure library call), so
        // this exercises the Nixie side and the seed-derivation logic even
        ...
        for (x, y) in a.iter().zip(b.iter()) {
            ...
            assert_eq!(x.nixie_result, y.nixie_result);
        }
    }
