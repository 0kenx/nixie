...
/// Execute an SMT2 file using Nixie solver with timeout protection
pub fn run_nixie(smt2_file: &Path) -> Result<SolverResult> {
    // Read the SMT2 file
...
mod tests {
    ...
    fn test_nixie_sat() -> Result<()> {
        ...
        let result = run_nixie(file.path())?;
        assert_eq!(result, SolverResult::Sat);
        Ok(())
    }

    #[test]
    fn test_nixie_unsat() -> Result<()> {
        let mut file = NamedTempFile::new()?;
        writeln!(file, "(set-logic QF_LIA)")?;
        writeln!(file, "(declare-const x Int)")?;
