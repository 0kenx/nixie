...
## Extracted correctness fix: audit boundaries

The safe CSR representation is retained. The removal primitive shifts only
within the selected physical span and lowers its live prefix length, leaving
its physical end unchanged. That behavior is correct and is why summing live
lengths in the bulk consumer was wrong. During compaction, preserve the old
end before overwriting it; the compacted write prefix never overtakes the
unread original span. Stable edge order and overflow-list order are retained.
The regression also exercises a second removal/compaction and dead IDs beyond
the supplied mask, which are fresh live edges under the factor-pass contract.

The two count/fill callers in `equiv.rs` were examined independently:
`refresh_binary_graph` uses the same live-binary predicate in both passes;
`rebuild_watches_and_binary_graph` counts the same live two-literal clauses it
later fills, with no clause mutation between passes. Phantom tick accounting
is separate and unchanged. Count and summed-count overflow now fail before
layout mutation; the fill guard validates the selected span, since an
otherwise-in-bounds write can overwrite a neighbor. The large-count test
injects counts directly and does not allocate billions of edges.

The factor consumer and ordinary retirement paths were also reviewed. Factor
compaction runs between its rounds and filters pass-local tombstones; fresh
IDs outside its dead mask remain live. Ordinary retirement and pure-literal
cleanup can leave physical gaps, which the representation explicitly permits.
The extracted fix repairs that shared contract without changing factoring
schedules, proof behavior or assignment order on valid unchanged inputs.
No benchmark wrong-answer reproducer is claimed: the demonstrated defects
are wrong edge placement after a legal operation sequence and insufficient
construction bounds, each protected directly by a regression.

Full workspace pre-parity checks pass: `cargo build --all-features`, 10846
nextest tests (13 existing skips), 111 doc tests, strict all-target/all-feature
Clippy, formatting and documentation with warnings denied. Verification uses
Rust 1.96.0 with the ordinary workspace profiles. To accommodate shared disk
pressure, completed test executables were removed only after the full suite
passed. A prior temporary DWARF-compression pass verified identical allocated
runtime sections before replacing a test artifact; production/benchmark
binaries were not altered. Logs and cleanup evidence are retained with the
qualified commit's binary cache.

Installed-Z3 **4.16.0** parity completed on the extracted fix: **174 Correct,
1 Inconclusive, 0 disagreements** across 175 cases. The inconclusive case
is `array_unique.smt2` (Nixie UNSAT, Z3 Unknown); Unknown is not counted as
agreement. The fresh environment snapshot and metadata are archived with
qualification evidence. Both ordinary release CLI and `stats_solve` binaries
are cached for the landed source. The five performance invocations above
remain the complete performance evidence for this round; the correctness fix
is not assigned an unmeasured throughput gain.
