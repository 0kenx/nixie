...
def run_cell(cnff: Path, seed: int, ref_verdict: str = "unknown"):
    ...
    env = dict(os.environ, DIAG="1", SEED=str(seed), PRINT_MODEL="1", NIXIE_EAGER_SUB="1")
    t0 = time.monotonic()
    try:
        ...
        rec = {
            ...
            "seed": seed,
            "config": {"id": "eager-sub", "flags": {"PRESET": "cadical", "SEED": str(seed), "EAGER_SUB": "1", "solver": "nixie"},
                       "cmdline": f"{BIN} {cnff}"},
            "arm": {"role": "treatment"},
...
def main():
    ...
    for f in files:
        ...
        flags = {"PRESET": "cadical", "SEED": "s", "EAGER_SUB": "1", "solver": "nixie"}
