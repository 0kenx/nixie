...
BIN = ROOT / "/tmp/wt-gent-off/target/release/examples/cnf_solve"
CORPUS = ROOT / "precompile/corpus-sc24f"
STORE = ROOT / "bench/suite/scripts/benchstore.py"
OUT = ROOT / "precompile/67b02c80/benchmark/runs/sc24f"
CAP_S = 60
SEEDS = [0, 1, 2, 3, 4]
HOST = {"id": "workstation", "cpu": "20 cores", "os": "Linux 7.2.2"}
GIT = {"sha_long": "67b02c80" + "0" * 32, "sha_short": "67b02c80", "dirty": True}
...
def run_cell(cnff: Path, seed: int, ref_verdict: str = "unknown"):
    ...
    try:
        ...
        rec = {
            ...
            "seed": seed,
            "config": {"id": "gent-off", "flags": {"PRESET": "cadical", "SEED": str(seed), "solver": "nixie"},
                       "cmdline": f"{BIN} {cnff}"},
            "arm": {"role": "treatment"},
...
def main():
    ...
    for f in files:
        ...
        flags = {"PRESET": "cadical", "SEED": "s", "solver": "nixie"}
