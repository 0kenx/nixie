#!/usr/bin/env python3
"""NIXIE_TIERED=1 treatment screen on the sc24f standing corpus (2026-09-12).
...
BIN = ROOT / "target/release/examples/cnf_solve"
CORPUS = ROOT / "precompile/corpus-sc24f"
STORE = ROOT / "bench/suite/scripts/benchstore.py"
OUT = ROOT / "precompile/6c769850-subfix/benchmark/runs/sc24f"
CAP_S = 60
SEEDS = [0, 1, 2, 3, 4]
HOST = {"id": "workstation", "cpu": "20 cores", "os": "Linux 7.2.2"}
GIT = {"sha_long": "6c769850" + "0" * 32, "sha_short": "6c769850", "dirty": True}
...
def run_cell(cnff: Path, seed: int, ref_verdict: str = "unknown"):
    ...
    env = dict(os.environ, DIAG="1", SEED=str(seed), PRINT_MODEL="1", NIXIE_ELIM_SUBFIX="1")
    t0 = time.monotonic()
    try:
        ...
        rec = {
            ...
            "seed": seed,
            "config": {"id": "elim-subfix", "flags": {"PRESET": "cadical", "SEED": str(seed), "ELIM_SUBFIX": "1", "solver": "nixie"},
                       "cmdline": f"{BIN} {cnff}"},
            "arm": {"role": "treatment"},
...
def main():
    ...
    for f in files:
        ...
        flags = {"PRESET": "cadical", "SEED": "s", "ELIM_SUBFIX": "1", "solver": "nixie"}
        ...
        for s in SEEDS:
            ...
            if next(OUT.glob(f"{f.name}__{sha8}__c{exact}__s{s}.json"), None) is not None:
                skipped += 1
                continue
            cells.append((f, s))
