...
BIN = ROOT / "precompile/303771ef/stats_solve"
CORPUS = ROOT / "precompile/corpus-sc24f"
STORE = ROOT / "bench/suite/scripts/benchstore.py"
OUT = ROOT / "precompile/303771ef/benchmark/runs/sc24f"
CAP_S = 60
SEEDS = [0, 1, 2, 3, 4]
HOST = {"id": "workstation", "cpu": "20 cores", "os": "Linux 7.2.2"}
GIT = {"sha_long": "303771ef" + "0" * 32, "sha_short": "303771ef", "dirty": False}
...
def run_cell(cnff: Path, seed: int, ref_verdict: str = "unknown"):
    ...
    env = dict(os.environ, DIAG="1", SEED=str(seed), PRINT_MODEL="1", NIXIE_TIERED="1")
    t0 = time.monotonic()
    try:
        ...
        rec = {
            ...
            "seed": seed,
            "config": {"id": "tiered", "flags": {"PRESET": "cadical", "SEED": str(seed), "TIERED": "1", "solver": "nixie"},
                       "cmdline": f"{BIN} {cnff}"},
            "arm": {"role": "treatment"},
