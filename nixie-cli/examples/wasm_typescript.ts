...
// Type definitions (these would normally come from nixie-wasm types)
interface NixieError {
    kind: string;
    message: string;
}
...
async function main(): Promise<void> {
    try {
        ...
        const nixieError = error as NixieError;
        if (nixieError.kind && nixieError.message) {
            console.error(`Nixie Error [${nixieError.kind}]: ${nixieError.message}`);
        } else {
