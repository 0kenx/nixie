...
/// because `nixie-solver` exposes no hook to snapshot them, and fabricating
...
mod tests {
    ...
    fn test_checkpoint_save_load() {
        use std::env;
        let temp_dir = env::temp_dir().join(format!("nixie_test_{}", std::process::id()));
    ...
    fn test_checkpoint_manager() {
        use std::env;
        let temp_dir = env::temp_dir().join(format!("nixie_test_mgr_{}", std::process::id()));
        let _ = fs::create_dir_all(&temp_dir);
    ...
    fn test_checkpoint_cleanup() {
        use std::env;
        let temp_dir = env::temp_dir().join(format!("nixie_test_cleanup_{}", std::process::id()));
        let _ = fs::create_dir_all(&temp_dir);
