//! Output formatting and display utilities for Nixie CLI
...
pub(crate) fn print_examples() {
    println!("Nixie SMT Solver - Practical Usage Examples");
    println!("==========================================\n");

    println!("BASIC USAGE");
    println!("-----------");
    println!("  # Solve a single SMT-LIB2 file");
    println!("  nixie problem.smt2\n");
    println!("  # Solve from stdin");
    println!("  echo '(check-sat)' | nixie\n");
    println!("  # Multiple files");
    println!("  nixie file1.smt2 file2.smt2 file3.smt2\n");
    println!("  # Glob patterns");
    println!("  nixie benchmarks/*.smt2\n");

    println!("INTERACTIVE MODE");
    println!("----------------");
    println!("  # Start interactive REPL");
    println!("  nixie --interactive\n");
    ...
    println!("  nixie problem.smt2 --analyze\n");
    println!("  # Get JSON analysis for programmatic use");
    println!("  nixie problem.smt2 --analyze --format json\n");
    println!("  # Classify problem and get solver recommendations");
    println!("  nixie problem.smt2 --classify\n");
    println!("  # Automatically tune solver based on problem");
    println!("  nixie problem.smt2 --auto-tune\n");

    println!("OUTPUT FORMATS");
    println!("--------------");
    println!("  # JSON output");
    println!("  nixie problem.smt2 --format json\n");
    println!("  # YAML output");
    println!("  nixie problem.smt2 --format yaml\n");
    println!("  # Save to file");
    println!("  nixie problem.smt2 -o result.txt\n");

    println!("PERFORMANCE & OPTIMIZATION");
    println!("--------------------------");
    println!("  # Parallel solving with 8 threads");
    println!("  nixie benchmarks/*.smt2 --parallel --threads 8\n");
    println!("  # Use fast preset for quick results");
    println!("  nixie problem.smt2 --preset fast\n");
    println!("  # Use thorough preset for maximum completeness");
    println!("  nixie problem.smt2 --preset thorough\n");
    println!("  # Enable result caching");
    println!("  nixie problem.smt2 --cache\n");
    println!("  # Set timeout (300 seconds)");
    println!("  nixie problem.smt2 --timeout 300\n");

    println!("RESOURCE LIMITS");
    println!("---------------");
    println!("  # Limit memory to 2GB");
    println!("  nixie problem.smt2 --memory-limit 2048\n");
    println!("  # Limit conflicts");
    println!("  nixie problem.smt2 --conflict-limit 10000\n");
    println!("  # Limit decisions");
    println!("  nixie problem.smt2 --decision-limit 50000\n");

    println!("ADVANCED FEATURES");
    println!("-----------------");
    println!("  # Enable preprocessing and simplification");
    println!("  nixie problem.smt2 --simplify\n");
    println!("  # Enumerate all models (up to 10)");
    println!("  nixie problem.smt2 --enumerate-models --max-models 10\n");
    println!("  # Validate proof for UNSAT results");
    println!("  nixie problem.smt2 --validate-proof\n");
    println!("  # Minimize satisfying model");
    println!("  nixie problem.smt2 --minimize-model\n");

    println!("UNSAT CORES & PROOFS");
    println!("--------------------");
    println!("  # Extract UNSAT core (minimal unsatisfiable subset)");
    println!("  nixie problem.smt2 --unsat-core\n");
    println!("  # Minimize UNSAT core");
    println!("  nixie problem.smt2 --unsat-core --minimize-core\n");
    println!("  # Generate proof tree visualization (DOT format)");
    println!("  nixie problem.smt2 --proof-dot proof.dot\n");
    println!("  # Convert DOT to PNG with GraphViz");
    println!("  nixie problem.smt2 --proof-dot proof.dot && dot -Tpng proof.dot -o proof.png\n");
    println!("  # Validate model against assertions");
    println!("  nixie problem.smt2 --validate-model\n");

    println!("INCREMENTAL SOLVING");
    println!("-------------------");
    println!("  # Enable incremental mode (supports push/pop)");
    println!("  nixie --incremental --interactive\n");
    ...
    println!("  nixie --completions bash > ~/.bash_completion.d/nixie\n");

    println!("COMBINED EXAMPLES");
    println!("-----------------");
    println!("  # Analyze, then solve with auto-tuning and statistics");
    println!("  nixie problem.smt2 --classify && nixie problem.smt2 --auto-tune --stats\n");
    println!("  # Process directory recursively with parallel solving and caching");
    println!("  nixie benchmarks/ --recursive --parallel --threads 16 --cache\n");
    println!("  # Quiet mode with JSON output for scripting");
    println!("  nixie problem.smt2 --quiet --format json\n");

    println!("\nFor more information, see: nixie --help");
}
...
pub(crate) fn print_version_info(args: &Args) {
    println_colored(
        args,
        &format!("Nixie SMT Solver v{}", env!("CARGO_PKG_VERSION")),
        Some(owo_colors::AnsiColors::Green),
    );
    println_colored(
        args,
...
pub(crate) fn print_help(args: &Args) {
    ...
    println_colored(args, "  - History is saved to ~/.nixie_history", None);
    ...
    println_colored(args, "  Example: nixie --lsp", None);
    println!();
    ...
    println_colored(args, "    nixie --completions bash   # For bash", None);
    println_colored(args, "    nixie --completions zsh    # For zsh", None);
    println_colored(args, "    nixie --completions fish   # For fish", None);
    println_colored(
        args,
        "    nixie --completions powershell  # For PowerShell",
        None,
    );
    println!();
    ...
    println_colored(
        args,
        "    nixie --completions bash > ~/.local/share/bash-completion/completions/nixie",
        None,
    );
    println!();
    ...
    println_colored(
        args,
        "  Nixie supports DIMACS CNF format for pure SAT problems:",
        None,
    );
    println_colored(
        args,
        "    nixie --dimacs problem.cnf        # Auto-detect CNF files",
        None,
    );
    println_colored(
        args,
        "    nixie --dimacs --dimacs-output problem.cnf  # DIMACS input/output",
        None,
    );
    println_colored(
        args,
        "    nixie --input-format dimacs file.txt  # Force DIMACS parsing",
        None,
    );
    println!();
    ...
    println_colored(
        args,
        "    nixie --memory-limit 1024 problem.smt2    # Limit to 1GB RAM",
        None,
    );
    println_colored(
        args,
        "    nixie --conflict-limit 10000 problem.smt2  # Max 10k conflicts",
        None,
    );
    println_colored(
        args,
        "    nixie --decision-limit 5000 problem.smt2   # Max 5k decisions",
        None,
    );
    println!();
    ...
    println_colored(
        args,
        "    nixie --validate-only problem.smt2        # Check syntax only",
        None,
    );
    println_colored(
        args,
        "    nixie --format-smtlib problem.smt2        # Pretty-print SMT-LIB2",
        None,
    );
    println_colored(
        args,
        "    nixie --format-smtlib --indent-width 4 problem.smt2  # Custom indentation",
        None,
    );
    println_colored(
        args,
    ...
    println_colored(
        args,
        "    nixie --preset fast problem.smt2          # Fast: optimize for speed",
        None,
    );
    println_colored(
        args,
        "    nixie --preset balanced problem.smt2      # Balanced: speed/completeness trade-off",
        None,
    );
    println_colored(
        args,
        "    nixie --preset thorough problem.smt2      # Thorough: maximize completeness",
        None,
    );
    println_colored(
        args,
        "    nixie --preset minimal problem.smt2       # Minimal: fastest, least processing",
        None,
    );
    println!();
    ...
    println_colored(
        args,
        "    nixie --simplify problem.smt2             # Enable preprocessing",
        None,
    );
    println_colored(
        args,
        "    nixie --minimize-model problem.smt2       # Find minimal model",
        None,
    );
    println_colored(
        args,
        "    nixie --validate-proof problem.smt2       # Validate UNSAT proofs",
        None,
    );
    println_colored(
        args,
        "    nixie --strategy portfolio problem.smt2   # Use portfolio strategy",
        None,
    );
    println_colored(
        args,
    ...
    println_colored(
        args,
        "    nixie --enumerate-models problem.smt2           # Find all models",
        None,
    );
    println_colored(
        args,
        "    nixie --enumerate-models --max-models 10 prob.smt2  # Max 10 models",
        None,
    );
    println!();
    ...
    println_colored(
        args,
        "    nixie --optimize problem.smt2                   # Enable optimization",
        None,
    );
    println!();
    ...
    println_colored(
        args,
        "    nixie --export-stats stats.csv problem.smt2     # Export as CSV",
        None,
    );
    println_colored(
        args,
        "    nixie --export-stats stats.json problem.smt2    # Export as JSON",
        None,
    );
    println_colored(
        args,
    ...
    println_colored(
        args,
        "    nixie --cache problem.smt2                      # Enable caching",
        None,
    );
    println_colored(
        args,
        "    nixie --cache-dir /path/to/cache problem.smt2   # Custom cache dir",
        None,
    );
    println_colored(
        args,
        "    nixie --benchmark-file bench.json problem.smt2  # Track performance",
        None,
    );
    println_colored(
        args,
        "    nixie --theory-opt lia:fastpath problem.smt2    # Theory optimization",
        None,
    );
    println_colored(
        args,
        "    nixie --enhanced-errors problem.smt2            # Better error messages",
        None,
    );
}
