//! Interactive mode (REPL) for Nixie SMT Solver
//!
//! This module provides an interactive Read-Eval-Print Loop (REPL) for the Nixie SMT solver,
...
pub(crate) fn run_interactive(ctx: &mut Context, args: &Args, verbosity: Verbosity) {
    ...
    let history_path = dirs::home_dir()
        .map(|mut p| {
            p.push(".nixie_history");
            p
        })
        .unwrap_or_else(|| PathBuf::from(".nixie_history"));
    ...
    loop {
        let readline = rl.readline("nixie> ");
        match readline {
            Ok(input) => {
