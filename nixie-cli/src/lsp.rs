//! LSP Server implementation for Nixie SMT Solver
...
/// LSP backend for Nixie SMT Solver
pub struct NixieBackend {
    client: Client,
                ...
                ')' => {
                    if paren_stack.is_empty() {
                        diagnostics.push(Diagnostic {
                            ...
                            code: None,
                            source: Some("nixie-lsp".to_string()),
...
impl LanguageServer for NixieBackend {
        ...
        Ok(InitializeResult {
            capabilities: ServerCapabilities {
                ...
                diagnostic_provider: Some(DiagnosticServerCapabilities::Options(
                    DiagnosticOptions {
                        identifier: Some("nixie-lsp".to_string()),
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.client
            .log_message(MessageType::INFO, "Nixie LSP server initialized")
            .await;
    }
...
pub async fn run_lsp_server() -> anyhow::Result<()> {
    ...
    let (service, socket) = LspService::new(NixieBackend::new);
