//! Nixie CLI - Command-line interface for Nixie SMT Solver
...
/// Nixie SMT Solver - Next-Generation SMT Solver in Pure Rust
...
struct Args {
    ...
    /// Cache directory path (default: ~/.nixie/cache)
    #[arg(long)]
    ...
    /// Use the ML-guided tactic selector (nixie-ml) to pick a solver posture
    ...
    /// Checkpoint directory (default: ~/.nixie/checkpoints)
    #[arg(long)]
...
/// Global allocator: mimalloc outperforms the system allocator on nixie's
...
async fn main() {
    ...
    if std::env::var_os(TIMEOUT_SUPERVISOR_GUARD).is_some() {
        ...
        let _ = std::thread::Builder::new()
            .name("nixie-orphan-guard".to_string())
            .spawn(move || {
...
pub(crate) fn apply_solver_options(ctx: &mut Context, args: &Args) {
    ...
    // `--memory-limit` has no corresponding lever in `nixie-solver` today: a
    ...
    // which are out of scope for `nixie-cli`, and the latter would need a
    ...
    if let Some(ref strategy) = args.strategy {
        ...
        match strategy.as_str() {
            // "cdcl" is the (only) search engine `nixie-solver` implements
    ...
    // `nixie-solver`/`nixie-cli`. Report that honestly instead of silently
    ...
    if args.optimize && !args.quiet {
        eprintln!(
            "warning: --optimize is accepted but objective optimization is not yet \
             implemented by nixie-cli; running a plain satisfiability check instead"
        );
    }
...
const TIMEOUT_SUPERVISOR_GUARD: &str = "NIXIE_TIMEOUT_SUPERVISED";
...
enum ScriptOutcome {
    ...
    /// the deadline.
    Finished(nixie_core::error::Result<Vec<String>>),
}
...
fn execute_script_with_timeout(
    ...
    let handle = match spawn_result {
        ...
        Err(e) => {
            ...
            return ScriptOutcome::Finished(Err(nixie_core::error::NixieError::Internal(format!(
    ...
    match rx.recv() {
        ...
        Err(std::sync::mpsc::RecvError) => {
            ...
            ScriptOutcome::Finished(Err(nixie_core::error::NixieError::Internal(
...
/// Reconstructs a [`nixie_core::sort::SortId`] from the subset of
...
fn resolve_basic_sort(ctx: &mut Context, sort_name: &str) -> Option<nixie_core::sort::SortId> {
    match sort_name {
...
/// `nixie_core::smtlib::parser::terms`), so a *separate* `execute_script`
...
fn enumerate_additional_models(ctx: &mut Context, max_models: usize) -> Vec<String> {
    ...
    if ctx.check_sat() != nixie_solver::SolverResult::Sat {
        ctx.pop();
    ...
    while found < requested_cap {
        ...
        match ctx.check_sat() {
            nixie_solver::SolverResult::Sat => {
                found += 1;
                lines.push(format!("; model {found}:"));
                lines.push(ctx.format_model());
            }
            nixie_solver::SolverResult::Unsat => {
                stop_reason = Some(format!(
            ...
            nixie_solver::SolverResult::Unknown => {
                stop_reason = Some(format!(
...
pub(crate) fn execute_and_format(ctx: &mut Context, script: &str, args: &Args) -> String {
    ...
    if args.validate_only {
        return match validate_script(script) {
            ...
            Err(e) => format!(
                "(error {})",
                nixie_core::smtlib::format_string_literal(&format!("Validation failed: {}", e))
            ),
        };
    }
        ...
        let algorithm =
            args.interpolate_algorithm
                ...
                .and_then(|a| match a.to_lowercase().as_str() {
                    "mcmillan" => Some(nixie_proof::InterpolationAlgorithm::McMillan),
                    "pudlak" => Some(nixie_proof::InterpolationAlgorithm::Pudlak),
                    "huang" => Some(nixie_proof::InterpolationAlgorithm::Huang),
                    _ => None,
                });
    ...
    if args.portfolio_mode || strategy_requests_portfolio || threads_request_parallel {
        ...
        match portfolio::solve_portfolio_custom(script, strategies, args, logic, ctx, timeout) {
            ...
            Err(e) => {
                return format!(
                    "(error {})",
                    nixie_core::smtlib::format_string_literal(&format!(
    ...
    match script_result {
        ...
        Err(e) => {
            ...
            if args.enhanced_errors {
                // `NixieError::enhance` (nixie-core) attaches a source-context
                // snippet, a "did you mean?" suggestion (against currently
                // declared symbols), and an actionable hint. It already
                // existed in nixie-core but `--enhanced-errors` never called
                ...
                format!(
                    "(error {})",
                    nixie_core::smtlib::format_string_literal(&enhanced.to_string())
                )
            } else {
                format!(
                    "(error {})",
                    nixie_core::smtlib::format_string_literal(&e.to_string())
                )
            }
        }
    }
