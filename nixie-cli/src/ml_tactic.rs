...
//!
//! This is the CLI's integration point for the `nixie-ml` crate: before a
...
const MODEL_PATH_ENV: &str = "NIXIE_ML_MODEL";

/// Resolve the on-disk model path (best-effort). Uses `NIXIE_ML_MODEL` when set,
/// otherwise the platform config dir under `nixie/`. Returns `None` when neither
...
mod tests {
    ...
    fn begin_produces_recommendation_comment() {
        ...
        let model = std::env::temp_dir().join(format!("nixie_ml_test_{}.json", std::process::id()));
