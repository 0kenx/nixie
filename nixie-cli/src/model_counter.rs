...
const WALL_CLOCK_BUDGET_OVERRIDE_MS_VAR: &str = "NIXIE_MODEL_COUNT_WALL_CLOCK_BUDGET_MS";
...
/// a [`nixie_core::sort::SortId`] so we can rebuild the variable's `TermId`
...
fn sort_id_from_name(ctx: &mut Context, sort_name: &str) -> Option<nixie_core::sort::SortId> {
    if sort_name == "Bool" {
...
mod tests {
    ...
    /// `NIXIE_TIMING_TESTS=1`; the underlying solve/count still runs and is
    ...
    fn test_exact_counting_unbounded_domain_is_bounded_not_pathological() {
        ...
        unsafe {
            std::env::set_var("NIXIE_MODEL_COUNT_WALL_CLOCK_BUDGET_MS", "200");
        }
        ...
        unsafe {
            std::env::remove_var("NIXIE_MODEL_COUNT_WALL_CLOCK_BUDGET_MS");
        }
