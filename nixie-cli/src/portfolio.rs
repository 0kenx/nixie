...
//!    `nixie-solver/src/solver/mod.rs`), so they genuinely change search
...
//!    (`nixie_sat::Solver::update_search_config`), so they genuinely change
...
use nixie_solver::{Context, RestartStrategy, SolverConfig, TheoryMode};
...
/// loop today versus stored for a future `nixie-solver` change, and for why
...
/// `nixie_solver::Context` does not expose an internal cancellation hook. The
...
mod tests {
    ...
    /// construction-time wiring `nixie-solver` does today (see module docs).
    #[test]
    ...
    fn test_solve_portfolio_custom_runs_real_solve() {
        ...
        let strategies = get_default_strategies();
        let args = Args::parse_from(["nixie"]);
        let ctx = Context::new();
