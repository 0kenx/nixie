//! File processing and execution logic for Nixie CLI
//!
...
/// `nixie_solver::SolverStats` is left at its default value on the accumulator.
fn accumulate_sat_stats(
    total: &mut nixie_solver::SolverStats,
    file_stats: &nixie_solver::SolverStats,
) {
...
fn process_files_sequential(
...
) -> (Vec<SolverResult>, nixie_solver::SolverStats) {
    ...
    let mut aggregated_sat_stats = nixie_solver::SolverStats::default();
    let start_time = Instant::now();
...
fn process_files_parallel(
    ...
    let per_file: Vec<(SolverResult, nixie_solver::SolverStats)> = files
        .par_iter()
        .map(|file| {
