//! REST API Server for Nixie SMT Solver
//!
...
pub async fn run_server(port: u16) -> anyhow::Result<()> {
    ...
    tracing::info!("Nixie REST API server listening on http://0.0.0.0:{}", port);
    println!("Nixie REST API server listening on http://0.0.0.0:{}", port);
...
async fn handle_version() -> impl IntoResponse {
    Json(VersionResponse {
        name: "Nixie SMT Solver".to_string(),
