...
//!
//! This module provides documentation and utilities for using nixie-wasm from JavaScript/TypeScript.
//! It includes:
//! - TypeScript type definition generation
//! - Usage examples and documentation
//! - Integration guidance for web applications
//!
//! # Using Nixie in JavaScript/TypeScript
//!
//! The `nixie-wasm` crate provides WebAssembly bindings for the Nixie SMT solver.
...
//!
//! ```typescript
//! import init, { WasmSolver, WasmErrorKind } from '@cooljapan/nixie';
//!
//! interface SMTModel {
//!     [key: string]: {
//!         name: string;
...
//!
//! ```bash
//! nixie --generate-wasm-types > nixie-types.d.ts
//! ```

use std::fs;
use std::path::Path;

/// TypeScript type definitions for nixie-wasm
///
/// These type definitions document the WASM API for TypeScript users.
pub const TYPESCRIPT_TYPES: &str = r#"// Nixie WASM TypeScript Type Definitions
// Generated by nixie-cli --generate-wasm-types
...
pub const EXAMPLE_JS_BASIC: &str = r#"// Basic Nixie WASM Usage Example
...
pub const EXAMPLE_JS_INCREMENTAL: &str = r#"// Incremental Solving with Nixie WASM
    ...
    try {
        ...
        switch (nixieError.kind) {
            ...
            case WasmErrorKind.InvalidInput:
                ...
                break;
            default:
                console.error("Solver error:", nixieError.message);
        }

        return { success: false, error: nixieError };
    } finally {
        solver.free();
    }
}
...
pub const EXAMPLE_JS_BITVECTORS: &str = r#"// Bitvector Operations with Nixie WASM
...
pub fn format_wasm_help() -> String {
    r#"Nixie WASM Integration Guide
============================

The nixie-wasm crate provides WebAssembly bindings for using Nixie in browsers and Node.js.
...
Generate TypeScript types:
  nixie --generate-wasm-types > nixie.d.ts

For more examples, see:
  nixie --examples

Documentation:
  https://docs.rs/nixie-wasm
"#
    .to_string()
}
...
mod tests {
    ...
    fn test_write_typescript_types() {
        let path = std::path::PathBuf::from("/tmp/nixie_test_types.d.ts");
    ...
    fn test_write_examples() {
        let dir = std::path::PathBuf::from("/tmp/nixie_test_examples");
    ...
    fn test_format_wasm_help() {
        ...
        assert!(help.contains("npm install @cooljapan/nixie"));
