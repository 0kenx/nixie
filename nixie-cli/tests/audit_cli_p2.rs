...
//!
//! 1. `nixie-cli/src/model_counter.rs` – `--count-models` used to always
...
//! 2. `nixie-cli/src/main.rs` – `--timeout` (and the config-file timeout)
...
/// Run `nixie` with `args`, killing it if it hasn't finished within
...
fn run_with_hard_kill(
    ...
    loop {
        ...
        if start.elapsed() > hard_kill_after {
            ...
            let _ = child.wait();
            panic!(
                "nixie did not exit within the hard-kill safety window ({:?}); \
