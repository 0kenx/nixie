...
/// solve, with no bug involved). Gate them behind `NIXIE_TIMING_TESTS=1` so the
...
fn timing_asserts_enabled() -> bool {
    env::var("NIXIE_TIMING_TESTS").as_deref() == Ok("1")
}
...
/// Regression test for the `NIXIE_TIMING_TESTS` gate: when disabled, a
/// wildly-over-budget duration must NOT panic – only the solve/output
/// assertions run.
///
/// This used to set/remove the real `NIXIE_TIMING_TESTS` process environment
...
/// mutates the real `NIXIE_TIMING_TESTS` environment variable.
#[test]
