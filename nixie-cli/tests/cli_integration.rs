//! Integration tests for nixie CLI
...
"#;
    ...
    let output = Command::new(nixie_bin())
        ...
        .output()
        .expect("Failed to execute nixie with --memory");
...
"#;
    ...
    let result = Command::new(nixie_bin())
        .arg("--output")
...
fn test_parallel_memory_aggregate() {
    ...
    let mut cmd = Command::new(nixie_bin());
    cmd.arg("--parallel").arg("--memory").arg("--quiet");
    for f in &files {
        cmd.arg(f.to_str().unwrap());
    }

    let output = cmd.output().expect("Failed to execute nixie");
...
fn test_exit_code_sat() {
    ...
    let status = Command::new(nixie_bin())
        .arg("--quiet")
        .arg(f.to_str().unwrap())
        .status()
        .expect("Failed to execute nixie");
