...
//!
//! This module provides infrastructure for testing nixie against SMT-LIB benchmarks.
...
fn run_smtlib_file(file: &Path) -> Result<(String, bool), String> {
    let output = Command::new(nixie_bin())
        ...
        .arg("--quiet")
        .output()
        .map_err(|e| format!("Failed to execute nixie: {}", e))?;
...
/// Extract nixie's actual reported answer from its CLI stdout: the first line
