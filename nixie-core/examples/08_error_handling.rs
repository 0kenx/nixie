...
//!
//! This example demonstrates Nixie's error handling system.
//! It covers:
...
//!
//! ## See Also
//! - [`NixieError`](nixie_core::error::NixieError) for error types
//! - [`Diagnostic`](nixie_core::diagnostics::Diagnostic)

use nixie_core::ast::TermManager;
use nixie_core::config::ResourceLimits;
use nixie_core::diagnostics::Diagnostic;
use nixie_core::error::{NixieError, Result, SourceLocation, SourceSpan};
use nixie_core::resource::{LimitStatus, ResourceManager};
use nixie_core::smtlib::parse_script;
use nixie_core::statistics::Statistics;
use std::time::Duration;

fn main() {
    println!("=== Nixie Core: Error Handling ===\n");

    // ======== Example 1: Basic Error Types ========
    println!("--- Example 1: NixieError Variants ---");

    // Create different error types
    let parse_err = NixieError::ParseError {
        position: 10,
        message: "Unexpected token ')'".to_string(),
    };
    let type_err = NixieError::SortMismatchSimple {
        expected: "Int".to_string(),
        found: "Bool".to_string(),
    };
    let unsupported_err = NixieError::Unsupported("Non-linear arithmetic".to_string());
    let internal_err = NixieError::Internal("Unexpected state".to_string());
    ...
    fn safe_divide(a: i32, b: i32) -> Result<i32> {
        if b == 0 {
            Err(NixieError::Internal("Division by zero".to_string()))
        } else {
            Ok(a / b)
        }
    }
    ...
    fn inner_operation() -> Result<i32> {
        Err(NixieError::Internal("Inner failure".to_string()))
    }
    ...
    println!("  2. NixieError provides specific error variants");
