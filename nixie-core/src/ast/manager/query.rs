...
//! fresh binder variable must avoid) and by `nixie-solver`'s MBQI
//! instantiation checking (`nixie_solver::mbqi::{sat_certify,
...
//! * `nixie-solver`'s MBQI grounding guard
//!   (`nixie_solver::mbqi::{sat_certify, integration}`), which rejects a
...
impl TermManager {
    ...
    /// * `nixie-solver`'s MBQI grounding guard, which rejects an
