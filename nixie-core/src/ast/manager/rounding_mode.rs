...
//!
//! `RoundingMode` is a first-class sort in Nixie (`SortKind::RoundingMode`),
...
//! existing; `nixie-solver`'s `context::rounding_mode` asserts both facts.
...
impl TermManager {
    ...
    /// the solver asserts it (see `nixie-solver`'s `context::rounding_mode`);
