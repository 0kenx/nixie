            ...
            let step = match &instr.kind {
                    ...
                    Some(_) => {
                        ...
                        match slot {
                            ...
                            Some(_) => Step::Goto(instr.next.ok_or_else(|| {
                                NixieError::EmatchError("Bind has no next instruction".to_string())
                            })?),
                            None => {
                ...
                InstructionKind::Check { property } => match manager.get(current_term) {
                    ...
                    Some(t) => {
                        ...
                        if passes {
                            Step::Goto(instr.next.ok_or_else(|| {
                                NixieError::EmatchError("Check has no next instruction".to_string())
                            })?)
                        } else {
                            Step::Fail
                        }
                ...
                InstructionKind::Ascend => {
                    let next = instr.next.ok_or_else(|| {
                        NixieError::EmatchError("Ascend has no next instruction".to_string())
                    })?;
                ...
                InstructionKind::MatchAny => {
                    // Match any term (wildcard)
                    Step::Goto(instr.next.ok_or_else(|| {
                        NixieError::EmatchError("MatchAny has no next instruction".to_string())
                    })?)
                }
