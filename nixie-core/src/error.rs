...
/// Main error type for Nixie operations
...
impl EnhancedError {
    ...
    #[must_use]
    pub fn new(error: NixieError) -> Self {
        Self {
            error,
            hint: None,
            did_you_mean: None,
...
impl NixieError {
    ...
    pub fn detailed_message(&self) -> String {
        match self {
            ...
            NixieError::UndefinedSymbol { location, symbol } => format!(
            ...
            NixieError::ResourceExhausted { reason } => format!(
    ...
    pub fn is_recoverable(&self) -> bool {
        matches!(
            ...
            NixieError::ParseError { .. }
                | NixieError::ParseErrorWithLocation { .. }
                | NixieError::UndefinedSymbol { .. }
        )
    }
    ...
    pub fn enhance(self, source: Option<&str>, known_symbols: &[&str]) -> EnhancedError {
        ...
        let hint = match &self {
            NixieError::ParseError { .. } | NixieError::ParseErrorWithLocation { .. } => {
            ...
            NixieError::UndefinedSymbol { symbol, .. } => Some(format!(
                "Declare '{symbol}' with 'declare-const' or 'declare-fun' before using it."
            )),
            NixieError::ArityMismatch { expected, .. } => {
        ...
        if let NixieError::UndefinedSymbol { symbol, .. } = &self
        ...
        if let Some(src) = source {
            let span = match &self {
                NixieError::ParseErrorWithLocation { location, .. }
                | NixieError::SortMismatch { location, .. }
                | NixieError::UndefinedSymbol { location, .. }
                | NixieError::TypeError { location, .. }
                | NixieError::ArityMismatch { location, .. }
                | NixieError::UnsupportedWithLocation { location, .. } => Some(location),
                _ => None,
            };
...
/// Result type alias using NixieError
pub type Result<T> = core::result::Result<T, NixieError>;
...
mod tests {
    ...
    fn test_error_constructors() {
        ...
        assert!(matches!(
            NixieError::sort_mismatch(span, "Int", "Bool"),
            NixieError::SortMismatch { .. }
        ));
        assert!(matches!(
            NixieError::parse_error(span, "unexpected token"),
            NixieError::ParseErrorWithLocation { .. }
        ));
        assert!(matches!(
            NixieError::undefined_symbol(span, "foo"),
            NixieError::UndefinedSymbol { .. }
        ));
        assert!(matches!(
            NixieError::type_error(span, "cannot apply"),
            NixieError::TypeError { .. }
        ));
        assert!(matches!(
            NixieError::arity_mismatch(span, 2, 3),
            NixieError::ArityMismatch { .. }
        ));
    }
    #[test]
    fn test_detailed_error_messages() {
        let span = SourceSpan::from_location(SourceLocation::new(5, 10, 42));
        let d = NixieError::sort_mismatch(span, "Int", "Bool").detailed_message();
        assert!(d.contains("Hint") && d.contains("Int") && d.contains("Bool"));
        let d = NixieError::undefined_symbol(span, "foo").detailed_message();
        assert!(d.contains("declare") && d.contains("foo"));
        let d = NixieError::arity_mismatch(span, 2, 3).detailed_message();
        assert!(d.contains("2") && d.contains("3"));
    }
    #[test]
    fn test_is_recoverable() {
        let span = SourceSpan::from_location(SourceLocation::new(5, 10, 42));
        assert!(NixieError::parse_error(span, "test").is_recoverable());
        assert!(NixieError::undefined_symbol(span, "foo").is_recoverable());
        assert!(!NixieError::Internal("test".to_string()).is_recoverable());
        assert!(!NixieError::sort_mismatch(span, "Int", "Bool").is_recoverable());
    }
    #[test]
    ...
    fn test_enhanced_error_display() {
        ...
        let enhanced = EnhancedError::new(NixieError::undefined_symbol(span, "foo_bar"))
    ...
    fn test_enhance_undefined_symbol_with_candidates() {
        ...
        let enhanced = NixieError::undefined_symbol(span, "varx")
    ...
    fn test_resource_exhausted_error() {
        let msg = NixieError::ResourceExhausted {
