...
impl ErrorContext {
    /// Create a new error context from an NixieError.
...
impl<T> ResultExt<T> for Result<T, NixieError> {
    fn context(self, context: impl Into<String>) -> Result<T, ErrorContext> {
...
mod tests {
    ...
    fn test_error_context_creation() {
        ...
        let err = NixieError::parse_error(span, "test error");
    ...
    fn test_add_suggestion() {
        let loc = SourceLocation::start();
        let span = crate::error::SourceSpan::from_location(loc);
        let err = NixieError::undefined_symbol(span, "x");
    ...
    fn test_related_terms() {
        let err = NixieError::Internal("test".to_string());
    ...
    #[test]
    fn test_result_ext() {
        fn might_fail() -> Result<i32, NixieError> {
            let loc = SourceLocation::start();
            let span = crate::error::SourceSpan::from_location(loc);
            Err(NixieError::parse_error(span, "test"))
        }
    ...
    fn test_chained_context() {
        fn inner() -> Result<i32, NixieError> {
            let loc = SourceLocation::start();
            let span = crate::error::SourceSpan::from_location(loc);
            Err(NixieError::parse_error(span, "inner error"))
        }
    ...
    fn test_format_full() {
        let loc = SourceLocation::start();
        let span = crate::error::SourceSpan::from_location(loc);
        let err = NixieError::undefined_symbol(span, "foo");
