...
use crate::error::{NixieError, SourceLocation, SourceSpan};
...
pub fn parse_error(location: SourceLocation, message: impl Into<String>) -> ErrorContext {
    ...
    ErrorContext::new(NixieError::parse_error(span, message.into()))
}
...
pub fn type_error(location: SourceLocation, message: impl Into<String>) -> ErrorContext {
    let span = SourceSpan::from_location(location);
    ErrorContext::new(NixieError::type_error(span, message.into()))
}
...
pub fn sort_mismatch(
    ...
    let span = SourceSpan::from_location(location);
    ErrorContext::new(NixieError::sort_mismatch(
        span,
        expected.into(),
        found.into(),
    ))
...
pub fn undefined_symbol(location: SourceLocation, symbol: impl Into<String>) -> ErrorContext {
    let span = SourceSpan::from_location(location);
    ErrorContext::new(NixieError::undefined_symbol(span, symbol.into()))
}
...
pub fn arity_mismatch(location: SourceLocation, expected: usize, found: usize) -> ErrorContext {
    let span = SourceSpan::from_location(location);
    ErrorContext::new(NixieError::arity_mismatch(span, expected, found))
}
...
pub fn validate_arity_range(
    ...
    if args.len() < min || args.len() > max {
        Err(ErrorContext::new(NixieError::Internal(format!(
...
mod tests {
    ...
    #[test]
    fn test_error_chain() {
        let base = NixieError::Internal("base error".to_string());
