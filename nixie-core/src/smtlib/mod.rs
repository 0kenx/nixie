...
//
// `pub` (not `pub(crate)`): reused outside `nixie-core` too, by any site in
// the workspace that emits SMT-LIB-shaped text containing a string value –
// e.g. `(error "...")` responses in `nixie-solver`/`nixie-cli` and proof
// metadata in `nixie-core::smtlib::printer::proof`, so a `"` or control
