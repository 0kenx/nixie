...
/// `RoundingMode`-sorted term. Nixie supports the two spellings a real
...
impl<'a> Parser<'a> {
    ...
    pub(super) fn parse_info_attribute_value(&mut self) -> Result<String> {
        ...
        if !matches!(first.kind, TokenKind::LParen) {
            ...
            return Self::attribute_token_text(first.kind).ok_or_else(|| NixieError::ParseError {
                position: start,
        ...
        while depth > 0 {
            ...
            match token.kind {
                ...
                other => {
                    ...
                    let text = Self::attribute_token_text(other).ok_or_else(|| {
    ...
    fn parse_command_step(&mut self) -> Result<CommandStep> {
        ...
        let cmd = match cmd_name.as_str() {
            ...
            "define-fun-rec" | "define-funs-rec" => {
                ...
                // `nixie_solver::context::recfun`). (Ported from upstream
                // v0.3.3.)
    ...
    fn parse_optional_numeral(&mut self, default: u32) -> Result<u32> {
        ...
        {
            return n.parse::<u32>().map_err(|_| NixieError::ParseError {
                position: token.start,
