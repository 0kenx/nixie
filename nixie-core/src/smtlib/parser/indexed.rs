...
impl Parser<'_> {
    ...
    pub(super) fn build_indexed_op(
        ...
        let single_index = |parts: &[String]| -> Result<u32> {
            ...
            parts[0].parse::<u32>().map_err(|_| NixieError::ParseError {
                position: 0,
        ...
        match name {
            ...
            "extract" => {
                ...
                let parse_index = |raw: &String| -> Result<u32> {
                    raw.parse::<u32>().map_err(|_| NixieError::ParseError {
                        position: 0,
                ...
                let width = self.bv_width(arg).ok_or_else(|| NixieError::ParseError {
                    position: 0,
            ...
            "divisible" => {
                ...
                let arg = single_arg(args)?;
                let n: BigInt = index_parts[0].parse().map_err(|_| NixieError::ParseError {
                    position: 0,
    ...
    fn fp_conv_format(&self, name: &str, index_parts: &[String]) -> Result<(u32, u32)> {
        ...
        let eb: u32 = eb_raw.parse().map_err(|_| NixieError::ParseError {
            position: self.lexer.position(),
            message: format!("invalid exponent-bits index for {name}: {eb_raw}"),
        })?;
        let sb: u32 = sb_raw.parse().map_err(|_| NixieError::ParseError {
            position: self.lexer.position(),
