...
/// unfolding in `nixie_solver::context::recfun`).
#[derive(Debug, Clone)]
...
pub struct Parser<'a> {
    ...
    #[allow(dead_code)]
    pub(super) errors: Vec<NixieError>,
...
impl<'a> Parser<'a> {
    ...
    /// `nixie-wasm` JS API or an `nixie-solver` `Context`) is rejected in script
    ...
    /// `nixie-solver` `Context` that lives in this crate) and via tests.
    #[allow(dead_code)]
    ...
    #[allow(dead_code)]
    fn record_error(&mut self, error: NixieError) -> Result<()> {
    ...
    #[must_use]
    #[allow(dead_code)]
    pub fn get_errors(&self) -> &[NixieError] {
        &self.errors
    }
