...
/// honest [`NixieError::ParseError`] instead of aborting the process.
...
impl<'a> Parser<'a> {
    ...
    pub(super) fn parse_sort_name(&mut self, name: &str) -> Result<SortId> {
        match name {
            ...
            // (nixie-solver, ...), so adding a bare variant here is a
            ...
            // (Ported from upstream v0.3.3.)
            "RegLan" => Err(NixieError::ParseError {
                position: self.lexer.position(),
                message: "sort 'RegLan' is a reserved SMT-LIB Strings theory sort name: it names \
                    the built-in encoding Nixie interns every regular-expression term at, so it \
        ...
        if let Some(token) = self.lexer.peek() {
            match &token.kind {
                ...
                TokenKind::LParen => {
                    ...
                    let next_token = self.lexer.peek().ok_or_else(|| NixieError::ParseError {
                        position: self.lexer.position(),
