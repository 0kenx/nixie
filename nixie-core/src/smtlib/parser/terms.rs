...
/// because the resulting [`NixieError::ParseError`] ("term nesting too deep") is
...
/// Symbol namespaces reserved by the SMT-LIB theories Nixie parses.
///
...
/// a theory operator Nixie has not implemented. Either way it must be reported
...
impl Parser<'_> {
    ...
    fn parse_leaf(&mut self, token: Token) -> Result<TermId> {
        match token.kind {
            ...
            TokenKind::Numeral(n) => {
                let value: BigInt = n.parse().map_err(|_| NixieError::ParseError {
            ...
            TokenKind::Binary(b) => {
                let value =
                    BigInt::parse_bytes(b.as_bytes(), 2).ok_or_else(|| NixieError::ParseError {
                        position: token.start,
            ...
            TokenKind::Decimal(d) => {
                let rational =
                    parse_decimal_to_rational(&d).map_err(|e| NixieError::ParseError {
                        position: token.start,
    ...
    fn accept_operand(&mut self, frame: &mut Frame, value: TermId) -> Result<bool> {
        match frame {
            ...
            Frame::Let(state) => {
                ...
                let name = state.pending.take().ok_or_else(|| NixieError::ParseError {
                    position: self.lexer.position(),
    ...
    fn close_frame(&mut self, frame: Frame) -> Result<TermId> {
        match frame {
            ...
            Frame::Let(state) => {
                ...
                let body = body.ok_or_else(|| NixieError::ParseError {
                    position: self.lexer.position(),
            ...
            Frame::Annot(state) => {
                ...
                let term = term.ok_or_else(|| NixieError::ParseError {
                    position: self.lexer.position(),
    ...
    fn operand_mismatch(&self, op: &str, got: usize) -> NixieError {
        NixieError::ParseError {
            position: self.lexer.position(),
    ...
    ///
    /// The solver's closure axiom (`nixie-solver`'s `context::rounding_mode`)
    ...
    fn open_flat_indexed_head(&mut self) -> Result<Opened> {
        ...
        if let Some(bv_val_str) = name.strip_prefix("bv") {
            ...
            let value: BigInt = bv_val_str.parse().map_err(|_| NixieError::ParseError {
                position: self.lexer.position(),
            ...
            _ => {
                    ...
                    && rest.chars().next().is_some_and(|c| c.is_ascii_digit())
                {
                    if let Some(dot_idx) = rest.find('.') {
                        ...
                        let value: BigInt = rest.parse().map_err(|_| NixieError::ParseError {
                            position: self.lexer.position(),
    ...
    ///    typo or an operator Nixie has not implemented.
    ///
            ...
            return Err(NixieError::ParseError {
                ...
                message: format!(
                    "unknown theory operator {display}: the symbol is in a reserved SMT-LIB \
                     theory namespace but Nixie does not implement it"
                ),
            });
        }
        Ok(())
