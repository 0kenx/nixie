...
use crate::error::{NixieError, Result, SourceSpan};
    ...
    match &term.kind {
        ...
        | TermKind::BvConcat(arg, _) => {
            if let Some(arg_term) = manager.get(*arg) {
                ...
                Err(NixieError::Internal(
                    "BV operand term not found".to_string(),
                ))
            }
        }
        ...
        TermKind::Store(array, _, _) => {
            if let Some(array_term) = manager.get(*array) {
                ...
                Err(NixieError::Internal("Array term not found".to_string()))
            }
        }
        ...
        TermKind::Let { body, .. } => {
            if let Some(body_term) = manager.get(*body) {
                ...
                Err(NixieError::Internal("Let body term not found".to_string()))
            }
        }
        ...
        | TermKind::FpRoundToIntegral(_, arg) => {
            if let Some(arg_term) = manager.get(*arg) {
                ...
                Err(NixieError::Internal(
                    "FP operand term not found".to_string(),
                ))
            }
        }
...
fn infer_arithmetic_sort(arg: TermId, manager: &TermManager) -> Result<SortId> {
    if let Some(term) = manager.get(arg) {
        let sort = manager
            .sorts
            .get(term.sort)
            .ok_or_else(|| NixieError::Internal(format!("Sort {} not found", term.sort.0)))?;
...
pub fn check_homogeneous_sorts(
    ...
    let first_term = manager
        .get(args[0])
        .ok_or_else(|| NixieError::Internal("First argument term not found".to_string()))?;
