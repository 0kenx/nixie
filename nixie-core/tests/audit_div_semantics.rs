...
use nixie_core::model::{EvalResult, Model, ModelEvaluator, Value};
use nixie_core::rewrite::arith::ArithRewriter;
use nixie_core::rewrite::{RewriteContext, Rewriter};
...
fn fold_div(a: i64, b: i64) -> (TermManager, nixie_core::ast::TermId) {
...
fn fold_mod(a: i64, b: i64) -> (TermManager, nixie_core::ast::TermId) {
    let mut m = TermManager::new();
    let mut ctx = RewriteContext::new();
    let mut rw = ArithRewriter::new();
    let la = m.mk_int(a);
...
fn expect_int(m: &TermManager, t: nixie_core::ast::TermId, expected: i64) {
...
fn eval(term: nixie_core::ast::TermId, m: &TermManager) -> EvalResult {
    let model = Model::new();
...
fn assert_term(script: &str) -> (TermManager, nixie_core::ast::TermId) {
    let mut m = TermManager::new();
...
fn bare_term_parse_stays_lenient() {
    ...
    let mut m = TermManager::new();
    let t = nixie_core::smtlib::parse_term("(< x y)", &mut m);
    assert!(
        t.is_ok(),
