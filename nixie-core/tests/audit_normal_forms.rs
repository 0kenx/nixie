...
//! `nixie-core/src/ast/normal_forms.rs::skolemize`.
//!
//! This is the Wave-1-deferred twin of the fixes already applied to
//! `SkolemizationTactic` in `nixie-core/src/tactic/quantifier.rs` (see
//! `nixie-core/tests/audit_tactic_quant.rs`). The standalone `skolemize`
...
use nixie_core::ast::{TermManager, skolemize, skolemize_with_counter};
...
fn p_apply_arg(manager: &TermManager, term: nixie_core::TermId) -> nixie_core::TermId {
...
fn contains_quantifier(manager: &TermManager, term_id: nixie_core::TermId) -> bool {
