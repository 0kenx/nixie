...
//! (`nixie-core/src/smtlib/parser/terms.rs`).
//!
...
/// order.
fn parse_asserts(script: &str) -> (TermManager, Vec<nixie_core::ast::TermId>) {
...
fn kind(manager: &TermManager, t: nixie_core::ast::TermId) -> TermKind {
...
fn eq_operands(
    m: &TermManager,
    t: nixie_core::ast::TermId,
) -> (nixie_core::ast::TermId, nixie_core::ast::TermId) {
    match kind(m, t) {
        TermKind::Eq(a, b) => (a, b),
...
fn bv_width(m: &TermManager, t: nixie_core::ast::TermId) -> Option<u32> {
...
fn concat_side(m: &TermManager, t: nixie_core::ast::TermId) -> nixie_core::ast::TermId {
...
/// The point of the whole exercise: an embedder calling Nixie from a worker
...
fn deeply_nested_annotations_survive_a_one_mib_stack() {
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            let term = nixie_core::smtlib::parse_term(&s, &mut manager)
...
fn control_bare_term_mode_still_allows_free_symbols() {
    ...
    let mut manager = TermManager::new();
    assert!(
        nixie_core::smtlib::parse_term("(some_free_fn a b)", &mut manager).is_ok(),
...
fn bare_term_mode_still_rejects_reserved_theory_namespace() {
    let mut manager = TermManager::new();
    assert!(
        nixie_core::smtlib::parse_term("(str.frobnicate a b c)", &mut manager).is_err(),
...
fn string_lit(m: &TermManager, t: nixie_core::ast::TermId) -> String {
    match kind(m, t) {
...
fn parse_one_assert(script: &str) -> (TermManager, nixie_core::ast::TermId) {
...
/// representable in Nixie's `char`-backed strings, so `str.from_code` declines
...
fn surrogate_escape_is_rejected() {
    // The SMT-LIB alphabet includes the UTF-16 surrogate range, which Nixie's
