...
//!
//! 1. `nixie_math::rational::carmichael_lambda` used `trial_division` with a
...
//! 2. `nixie_math::grobner::buchberger::NraSolver::check_sat` used to report
...
//! `#[cfg(test)]` module inside `nixie-math/src/grobner/buchberger.rs`
//! itself.
//!
//! A third finding in this package's scope --
//! `nixie-core/src/qe/array/mod.rs` no longer re-exporting the disabled
//! placeholder `quantifier_elim` types -- is a pure API-surface change in a
//! different crate (`nixie-core`) with no runtime behavior to regression-test
//! from here; it is covered by `cargo check -p nixie-core` producing zero
//! warnings (no more dead/unused placeholder re-export) and by
//! `nixie-core/src/qe/array/quantifier_elim.rs`'s own existing tests.

use nixie_math::grobner::buchberger::{PolynomialConstraint, Relation};
use nixie_math::grobner::{NraSolver, SatResult};
use nixie_math::polynomial::Polynomial;
use nixie_math::rational::{carmichael_lambda, euler_totient, is_prime};
