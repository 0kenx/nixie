# nixie-ml – Machine Learning-Guided Heuristics for Nixie

Machine learning-guided heuristics for the Nixie SMT solver, providing adaptive branching, clause deletion, restart policies, and tactic selection driven by learned models trained on solving experience.
...
Add `nixie-ml` to your workspace member and enable it from `nixie-solver`:

```toml
[dependencies]
nixie-ml = { workspace = true }
```

### Enabling ML Branching

```rust
use nixie_ml::branching::{MlVsids, BranchingFeatures};
use nixie_ml::training::online_learning::OnlineLearner;
...
```rust
use nixie_ml::tactic::{FormulaFeatures, TacticSelector, TacticPortfolio};
...
```rust
use nixie_ml::training::{DataCollector, OfflineTrainer};
