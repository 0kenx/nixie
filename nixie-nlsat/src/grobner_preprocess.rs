...
use nixie_math::grobner::{grobner_basis, reduce};
...
impl GroebnerPreprocessor {
    ...
    /// (in `nixie-math`, outside this crate) – but it does cap the worst-case
...
fn isolate_variable(poly: &Polynomial, var: Var) -> Option<Polynomial> {
    ...
    let other_poly = Polynomial::from_terms(
        other_terms,
        nixie_math::polynomial::MonomialOrder::default(),
    );
...
mod tests {
    ...
    fn test_stats_tracking() {
        ...
        let outer = nixie_time::Instant::now();
    ...
    fn test_grobner_thread_counter_released_after_completion() {
        ...
        for _ in 0..200 {
            ...
            std::thread::sleep(nixie_time::Duration::from_millis(5));
        }
        assert!(
            released,
