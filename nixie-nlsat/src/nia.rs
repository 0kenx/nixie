...
impl NiaSolver {
    ...
    fn is_integer_solution(&self, model: &Model) -> bool {
        for var in 0..self.nlsat.num_arith_vars() {
            ...
            // absent (see `nixie_nlsat::solver::Model`). Without this check the
    ...
    /// fabricated "row" for `nixie_math::lp::cutting_planes::
    ...
    /// subsystem, which is out of scope here. `nixie-theories`'s LIA solver
    ...
    /// `nixie_theories::arithmetic::nla`; until then, branch-and-bound alone
