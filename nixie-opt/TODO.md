...
## Dependencies
- **nixie-core**: Term representation, constraints
- **nixie-sat**: Core-guided MaxSAT solving
- **nixie-solver**: OMT SMT solving (needs integration)

## Provides (enables other crates)
- **nixie-solver**: Optimization objectives (minimize/maximize)
- **nixie-cli/nixie-wasm**: MaxSMT/OMT commands
...
- [x] Code quality improvements:
  ...
  - Maintained zero warnings policy for nixie-opt
...
- [x] `OptContext::check_sat` – replaced `Unknown` stub with real SAT check via `nixie_solver::Solver`; model snapshot with `Model::eval`.
...
- [x] `OptContext::optimize_single_objective` – replaced `Unknown` stub by delegating to `nixie_solver::Optimizer::{minimize,maximize}`.
