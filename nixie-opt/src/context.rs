...
use nixie_solver::{Model, Solver, SolverResult};
...
impl OptContext {
    ...
    fn deadline(&self) -> Option<nixie_time::Instant> {
        if self.config.timeout_ms == 0 {
            return None;
        }
    ...
    fn deadline_passed(deadline: Option<nixie_time::Instant>) -> bool {
        deadline.is_some_and(|d| nixie_time::Instant::now() >= d)
    }
    ...
    /// rather than delegating to `nixie_solver::Optimizer`, because the latter
    ...
    /// `nixie_solver::Optimizer::optimize_int`), with periodic `deadline` checks
    ...
    /// search (mirrors `nixie_solver::Optimizer::optimize_real`), with periodic
    ...
    /// [`Self::optimize_single_objective`]: `nixie_solver::Optimizer` offers no
