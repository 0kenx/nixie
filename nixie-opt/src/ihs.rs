...
use nixie_sat::{Lit, Solver as SatSolver, SolverResult, Var};
...
pub struct IhsSolver {
    ...
    next_hs_var: u32,
    /// Best model found
    best_model: Option<Vec<nixie_sat::LBool>>,
}
...
impl IhsSolver {
            ...
            match result {
                SolverResult::Sat => {
                    ...
                    for (&id, &hs_var) in &self.soft_to_hs_var {
                        ...
                        if var_idx < model.len() && model[var_idx] == nixie_sat::LBool::True {
    ...
    /// Get the best model
    pub fn best_model(&self) -> Option<&[nixie_sat::LBool]> {
