...
fn omt_deadline(timeout_ms: u64) -> Option<nixie_time::Instant> {
    if timeout_ms == 0 {
        return None;
    }
...
fn omt_deadline_passed(deadline: Option<nixie_time::Instant>) -> bool {
...
mod tests {
    ...
    fn test_omt_binary_search_honors_timeout() {
        ...
        let checker = |c: &ArithConstraint| -> Option<FxHashMap<u32, BigRational>> {
            std::thread::sleep(nixie_time::Duration::from_millis(10));
        ...
        assert!(
            elapsed < nixie_time::Duration::from_secs(5),
