...
//!
//! 1. `nixie-opt/src/maxsat/algorithms.rs`: the default weighted (stratified)
...
//! 2. `nixie-opt/src/preprocess.rs`: unit propagation must never treat a SOFT
//!    unit clause as a hard fact.
//! 3. `nixie-opt/src/context.rs`: `OptContext::optimize()` must not coerce
...
fn preprocess_never_treats_soft_unit_as_fact() {
    use nixie_opt::{PreprocessConfig, SoftClause, SoftId};
...
fn optctx_rational_weight_is_not_coerced_to_one() {
    ...
    assert!(matches!(
        result,
        nixie_opt::OptResult::Optimal | nixie_opt::OptResult::Satisfiable
    ));
