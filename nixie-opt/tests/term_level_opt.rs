//! Integration tests for the OptContext → nixie-solver pipeline.
//!
...
use nixie_opt::{ModelValue, OptConfig, OptContext, OptResult, Weight};
use num_bigint::BigInt;

// ======== helpers ========

/// Extract `Some(BigInt)` from an `OptContext` model for a variable term.
fn model_int(ctx: &OptContext, term: nixie_core::ast::TermId) -> Option<BigInt> {
...
// delegated wholesale to `nixie_solver::Optimizer`, which has no timeout hook
