...
/// ```no_run
/// use std::path::Path;
/// use nixie_proof::replay::{ProofReplayer, VerificationResult};
///
/// let result = ProofReplayer::replay_from_file(Path::new("/tmp/proof.nixielog"))
...
mod tests {
    ...
    fn temp_path(name: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!("nixie_replay_{}.nixielog", name))
    }
