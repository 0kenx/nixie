//! Integration with nixie-sat for DRAT proof generation
//!
//! This module provides utilities to integrate DRAT proof generation
//! from the nixie-proof crate with the nixie-sat SAT solver.

#[cfg(feature = "sat-integration")]
use crate::drat::{Clause, DratProof, DratProofProducer, Lit};

/// Wrapper that combines nixie-sat Solver with DRAT proof generation
///
...
impl ProofRecordingSolver {
    ...
    #[must_use]
    pub fn with_config(config: nixie_sat::SolverConfig) -> Self {
        Self {
            solver: nixie_sat::Solver::with_config(config),
            proof: None,
        }
    }
    ...
    #[must_use]
    pub fn solver(&self) -> &nixie_sat::Solver {
        &self.solver
    }

    /// Get a mutable reference to the underlying solver
    pub fn solver_mut(&mut self) -> &mut nixie_sat::Solver {
        &mut self.solver
    }

    /// Add a clause to the solver and proof
    pub fn add_clause(&mut self, lits: &[nixie_sat::Lit]) {
        // Convert nixie_sat::Lit to our Lit format
    ...
    pub fn delete_clause(&mut self, lits: &[nixie_sat::Lit]) {
        let proof_lits: Vec<Lit> = lits.iter().map(|l| l.to_dimacs()).collect();
    ...
    pub fn solve(&mut self) -> nixie_sat::SolverResult {
        self.solver.solve()
    }
}
...
/// Convert nixie-sat literal to DRAT literal format
///
/// This is a utility function for converting between the two formats.
#[cfg(feature = "sat-integration")]
#[must_use]
pub fn sat_lit_to_drat(lit: nixie_sat::Lit) -> Lit {
    lit.to_dimacs()
}

/// Convert DRAT literal to nixie-sat literal format
///
/// This is a utility function for converting between the two formats.
#[cfg(feature = "sat-integration")]
#[must_use]
pub fn drat_lit_to_sat(lit: Lit) -> nixie_sat::Lit {
    nixie_sat::Lit::from_dimacs(lit)
}

/// Convert a clause from nixie-sat format to DRAT format
#[cfg(feature = "sat-integration")]
#[must_use]
pub fn sat_clause_to_drat(clause: &[nixie_sat::Lit]) -> Clause {
    clause.iter().map(|&lit| sat_lit_to_drat(lit)).collect()
}

/// Convert a clause from DRAT format to nixie-sat format
#[cfg(feature = "sat-integration")]
#[must_use]
pub fn drat_clause_to_sat(clause: &Clause) -> Vec<nixie_sat::Lit> {
