"""
Type stubs for the Nixie SMT Solver Python bindings.

Nixie is a high-performance SMT (Satisfiability Modulo Theories) solver
...
Quick start (z3-python parity API)::
    ...
    a = ctx.bool_const("a")
    b = ctx.bool_const("b")
    solver.add(nixie.And(a, b), ctx.tm)
    solver.add(nixie.Or(a, nixie.Not(b)), ctx.tm)
    solver.add(nixie.Implies(a, b), ctx.tm)
    solver.add(nixie.If(a, x, y) > ctx.int_val(0), ctx.tm)
"""
...
def If(cond: Term, then_: Term, else_: Term) -> Term:
    ...
    Example::
        ...
        result = nixie.If(cond, then_val, else_val)
    """
    ...
...
def ArraySort(tm: TermManager, index_sort: Sort, elem_sort: Sort) -> Sort:
    ...
    Example::
        ...
        arr_sort = nixie.ArraySort(tm, nixie.IntSort(tm), nixie.IntSort(tm))
    """
    ...
...
def FPSort(tm: TermManager, eb: int, sb: int) -> Sort:
    ...
    Example::
        ...
        fp16 = nixie.FPSort(tm, 5, 11)   # IEEE 754 half-precision
        fp32 = nixie.FPSort(tm, 8, 24)   # IEEE 754 single-precision
    """
    ...
...
def FPVal(tm: TermManager, sign: bool, exp: int, sig: int, sort: Sort) -> Term:
    ...
    Example::
        ...
        one  = nixie.FPVal(tm, False, 127, 0, sort)   # +1.0 in fp32
    """
    ...
...
def ForAll(tm: TermManager, vars: List[tuple[str, str]], body: Term) -> Term:
    ...
    Example::
        ...
        fml = nixie.ForAll(ctx.tm, [("x", "Int")], body)
    """
    ...
...
def Exists(tm: TermManager, vars: List[tuple[str, str]], body: Term) -> Term:
    ...
    Example::
        ...
        x = ctx.int_const("x")
        body = x > ctx.int_val(0)
        fml = nixie.Exists(ctx.tm, [("x", "Int")], body)
    """
    ...
