"""Tests for array theory support in Nixie Python bindings.
...
def test_array_sort_int_bool():
    ...
    tm = nixie.TermManager()
    int_s = nixie.IntSort(tm)
    bool_s = nixie.BoolSort(tm)
    arr_s = nixie.ArraySort(tm, int_s, bool_s)
    assert arr_s is not None
...
def test_array_sort_same_config_dedup():
    ...
    tm = nixie.TermManager()
    int_s = nixie.IntSort(tm)
    s1 = nixie.ArraySort(tm, int_s, int_s)
    s2 = nixie.ArraySort(tm, int_s, int_s)
    assert s1 == s2
