...
`nixie-py/src/solver_py.rs`) is covered directly and exhaustively at the Rust
...
feature - see `nixie-py/src/term.rs`), so `test_bv_mk_bv_accepts_literal_wider_than_i64`
...
this fix and out of scope for the nixie-py package. See the session notes
for that finding.
"""
...
def test_bv_mk_bv_accepts_literal_wider_than_i64():
    ...
    (regression test for infra-final nixie-py: mk_bv used to take `i64`,
