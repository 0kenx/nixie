"""Tests for quantifier support in Nixie Python bindings.

Covers:
- nixie.ForAll(tm, vars, body)   – universal quantifier combinator
- nixie.Exists(tm, vars, body)   – existential quantifier combinator
...
def test_forall_exists_at_module_level():
    """ForAll and Exists are now exposed in the nixie Python module."""
    assert hasattr(nixie, "ForAll")
    assert hasattr(nixie, "Exists")


def test_existing_boolean_combinators_still_present():
    """The boolean combinators that were already available are still there."""
    assert hasattr(nixie, "And")
    assert hasattr(nixie, "Or")
    assert hasattr(nixie, "Not")
    assert hasattr(nixie, "Implies")
    assert hasattr(nixie, "If")
...
def test_forall_single_bool_var():
    ...
    b = ctx.bool_const("b")
    body = b
    fml = nixie.ForAll(ctx.tm, [("b", "Bool")], body)
    assert fml is not None
...
def test_forall_multi_var():
    ...
    fml = nixie.ForAll(ctx.tm, [("x", "Int"), ("y", "Int")], body)
    assert fml is not None
...
def test_exists_single_bool_var():
    ...
    ctx = nixie.Context()
    b = ctx.bool_const("b")
    fml = nixie.Exists(ctx.tm, [("b", "Bool")], b)
    assert fml is not None
...
def test_exists_multi_var():
    ...
    y = ctx.int_const("y")
    body = x > y
    fml = nixie.Exists(ctx.tm, [("x", "Int"), ("y", "Int")], body)
    assert fml is not None
...
def test_forall_and_exists_are_distinct():
    ...
    x = ctx.int_const("x")
    body = x > ctx.int_val(0)
    fa = nixie.ForAll(ctx.tm, [("x", "Int")], body)
    ex = nixie.Exists(ctx.tm, [("x", "Int")], body)
    assert fa != ex
...
def test_nested_forall_exists():
    ...
    y = ctx.int_const("y")
    body = x > y
    inner = nixie.Exists(ctx.tm, [("y", "Int")], body)
    outer = nixie.ForAll(ctx.tm, [("x", "Int")], inner)
...
def test_forall_various_sorts(sort_name):
    ...
    fml = nixie.ForAll(tm, [("v", sort_name)], body)
    assert fml is not None
...
def test_exists_various_sorts(sort_name):
    ...
    tm = nixie.TermManager()
    body = tm.mk_bool(True)
    fml = nixie.Exists(tm, [("v", sort_name)], body)
    assert fml is not None
...
def test_forall_unknown_sort_raises():
    ...
    with pytest.raises((ValueError, Exception)):
        nixie.ForAll(tm, [("v", "UnknownSort")], body)
