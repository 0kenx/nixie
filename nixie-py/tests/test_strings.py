"""Tests for string theory support in Nixie Python bindings.
...
def test_string_sort_exists():
    ...
    assert hasattr(nixie, "StringSort")
...
def test_string_val_at_module_level():
    """nixie.StringVal(tm, s) creates a string-literal Term."""
    tm = nixie.TermManager()
    t = nixie.StringVal(tm, "hello")
...
def test_concat_at_module_level():
    """nixie.Concat(tm, s1, s2) produces a Term."""
    tm = nixie.TermManager()
    s1 = tm.mk_string_val("hello")
    s2 = tm.mk_string_val(" world")
    cat = nixie.Concat(tm, s1, s2)
...
def test_length_at_module_level():
    """nixie.Length(tm, s) produces a Term (integer-sorted)."""
    tm = nixie.TermManager()
...
def test_contains_at_module_level():
    """nixie.Contains(tm, s, sub) produces a boolean Term."""
    tm = nixie.TermManager()
...
def test_prefix_of_at_module_level():
    """nixie.PrefixOf(tm, pre, s) produces a boolean Term."""
    tm = nixie.TermManager()
...
def test_suffix_of_at_module_level():
    """nixie.SuffixOf(tm, suf, s) produces a boolean Term."""
    tm = nixie.TermManager()
...
def test_contains_prefix_suffix_are_distinct():
    ...
    c = nixie.Contains(tm, s, sub)
    p = nixie.PrefixOf(tm, sub, s)
    sf = nixie.SuffixOf(tm, sub, s)
    assert c != p
    assert c != sf
    assert p != sf
