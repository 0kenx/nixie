...
## Dependencies
- **nixie-core**: Term/literal representation

## Provides (enables other crates)
- **nixie-solver**: CDCL(T) SAT core
- **nixie-opt**: MaxSAT core algorithms
- **nixie-proof**: DRAT/LRAT proof integration
- **nixie-nlsat**: Boolean reasoning component

---

## Beyond Z3: Native Parallelism

Nixie's SAT solver includes modern parallel solving techniques:
...
Z3 has limited parallelism; Nixie scales with multi-core hardware.
...
- [x] `DratProof` renamed to `DratWriter<W>` – now generic over any `W: Write + Send`; `DratProof` name collided with `nixie-proof::DratProof`
