...
mod tests {
    ...
    fn text_add_and_delete_format() {
        let mut t = DratTracer::open("/tmp/_nixie_drat_text.drat").unwrap();
        t.add_clause(&[1, -2, 3]);
        t.delete_clause(7, false, &[4, 5]);
        t.close(false);
        drop(t);
        let s = std::fs::read_to_string("/tmp/_nixie_drat_text.drat").unwrap();
        assert_eq!(s, "1 -2 3 0\nd 4 5 0\n");
    }

    #[test]
    fn binary_add_and_delete_format() {
        let mut t = DratTracer::open_binary("/tmp/_nixie_drat_bin.drat").unwrap();
        t.add_clause(&[1, -2, 3]);
        t.delete_clause(7, false, &[4, 5]);
        t.close(false);
        drop(t);
        let b = std::fs::read("/tmp/_nixie_drat_bin.drat").unwrap();
        let mut i = 0;
        // add: 'a' 1 -2 3 0
        assert_eq!(b[i], b'a');
        i += 1;
    ...
    fn empty_clause_emits_lone_zero() {
        let mut t = DratTracer::open("/tmp/_nixie_drat_empty.drat").unwrap();
        t.add_empty();
        t.close(false);
        drop(t);
        let s = std::fs::read_to_string("/tmp/_nixie_drat_empty.drat").unwrap();
        assert_eq!(s, "0\n");
    }
}
