...
mod tests {
    ...
    fn text_add_line_format() {
        let mut t = LratTracer::open("/tmp/_nixie_lrat_text.lrat").unwrap();
        t.add_clause(7, &[1, -2, 3], &[4, 5]);
        t.close(false);
        drop(t);
        let s = std::fs::read_to_string("/tmp/_nixie_lrat_text.lrat").unwrap();
        assert_eq!(s, "7 1 -2 3 0 4 5 0\n");
    }

    #[test]
    fn text_deferred_deletion_batches_into_one_line() {
        let mut t = LratTracer::open("/tmp/_nixie_lrat_del.lrat").unwrap();
        t.delete_clause(2, false, &[]);
        t.delete_clause(5, false, &[]);
        t.add_clause(9, &[1], &[]); // flushes the deferred 'd' line first
        t.close(false);
        drop(t);
        let s = std::fs::read_to_string("/tmp/_nixie_lrat_del.lrat").unwrap();
    ...
    fn binary_add_line_format() {
        let mut t = LratTracer::open_binary("/tmp/_nixie_lrat_bin.lrat").unwrap();
        t.add_clause(1, &[1, -1], &[]); // id 1, lits 1,-1, no hints
        t.close(false);
        drop(t);
        let b = std::fs::read("/tmp/_nixie_lrat_bin.lrat").unwrap();
        let mut i = 0;
        assert_eq!(b[i], b'a');
        i += 1;
    ...
    #[test]
    fn varint_roundtrip() {
        let mut t = LratTracer::open_binary("/tmp/_nixie_lrat_v.lrat").unwrap();
        // lit 100 → 200, lit -100 → 201; id 1 → 2.
        t.add_clause(1, &[100, -100], &[]);
        t.close(false);
        drop(t);
        let b = std::fs::read("/tmp/_nixie_lrat_v.lrat").unwrap();
        let mut i = 1; // skip 'a'
        assert_eq!(decode_id(read_varint(&b, &mut i)), 1);
