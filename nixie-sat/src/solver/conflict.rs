...
impl Solver {
    ...
    pub(super) fn analyze(&mut self, conflict: ClauseId) -> (u32, SmallVec<[Lit; 32]>) {
        ...
        // historical nixie behavior – both structures bumped on every conflict
    ...
    /// cadical is safe because its `vidx` is already 1-based, but nixie vars are
    ...
    /// Debug instrumentation for the shrink study (NIXIE_SHRINK_TRACE=1).
    fn shrink_trace_enabled(&self) -> bool {
        ...
        #[cfg(feature = "std")]
        {
            ...
            *FLAG.get_or_init(|| std::env::var("NIXIE_SHRINK_TRACE").is_ok())
        }
        #[cfg(not(feature = "std"))]
        {
            false
    ...
    /// search shape.  `NIXIE_SHRINK_NULL` runs the full shrink walk and
    ...
    /// plain level).  Matched null (`NIXIE_CHRONOREUSE_NULL=1`): the bump key
...
mod tests {
    ...
    // `VarInfo.level` left stale at the level it last held. Two Nixie z3-parity
