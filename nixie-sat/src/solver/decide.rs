...
/// Whether decision/conflict tracing is enabled (env var `NIXIE_TRACE_DECISIONS`).
///
...
impl Solver {
    ...
    /// Emit one decision-trace line to stderr when the `NIXIE_TRACE_DECISIONS`
    /// environment variable is set.
    ///
    /// Tab-separated format, one line per decision:
    /// `nixie-dec <decision#> <level> <var> <src> <pol>`
    /// where `<src>` is the [`BranchSource`] (`pick_branch_var` set
    /// `last_branch_source`) and `<pol>` is `1` for positive / `0` for
    /// negative. Correlate `<var>` with the `nixie-varlegend` lines the SMT
    /// solver emits (see `nixie-solver`) to classify each decision's atom.
    ///
    ...
    /// Emit one conflict-trace line when `NIXIE_TRACE_DECISIONS` is set.
    ///
    ...
    ///
    /// Tab-separated: `nixie-conflict <path> <level> <learnt_len> <props>` where
    ...
    pub(super) fn trace_conflict(&self, path: &str, level: u32, learnt_len: usize, backjump: u32) {
        ...
        eprintln!(
            "nixie-conflict\t{path}\t{level}\t{learnt_len}\t{}\t{backjump}\t{}",
    ...
    pub(super) fn backtrack(&mut self, level: u32) -> usize {
        ...
        // `NIXIE_XORSEARCH` installed the state.
        self.xor_search_rollback();
    ...
    /// values.  Nixie saves phases at *unassignment* time, so the array can be
