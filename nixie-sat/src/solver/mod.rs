...
pub struct SolverConfig {
    ...
    /// as A/B infrastructure with `NIXIE_CHRONO_REUSE_AFTER`;
    /// `0` = always active when `chrono_reuse` is on.
    pub chrono_reuse_after: u64,
    /// Debug knob (`NIXIE_CHRONO_ALWAYS=1`): force chronological backtracking
    ...
    /// Default OFF in Nixie pending measurement (see the walk study).
...
impl Default for SolverConfig {
    fn default() -> Self {
        Self {
            ...
            walk_nonstable: true,
            walk_effort: 80,
            walk_warmup: std::env::var("NIXIE_WARMUP").is_ok_and(|v| v != "0"),
            reuse_trail: true,
...
/// `Solver::trace_decision`, gated on `NIXIE_TRACE_DECISIONS`) can report the
...
pub struct Solver {
    ...
    /// cadical-reduce port (`NIXIE_CADICAL_REDUCE`, study in
    ...
    /// Faithful-stabilize port (`NIXIE_STAB_FAITHFUL`, study in
    ...
    /// Scratch for the shuffled-phase-length null (`NIXIE_STAB_NULL`): the
    ...
    /// `nixie-cli --timeout`-style bounded solving.
    ...
    /// Shrink-study accumulator (NIXIE_SHRINK_TRACE=1):
    ...
    /// Shrink-study failure-reason counters (NIXIE_SHRINK_TRACE=1).
    ...
    /// `NIXIE_REPHASE_OFF`): the post-rephase restart-consistency check must
    ...
    /// `NIXIE_GLUE_NULL` matched null feeds into the restart EMAs (same
...
impl Solver {
    ...
    ///
    /// The `NIXIE_CHRONO_REUSE` / `NIXIE_CHRONO_ALWAYS` experiment overrides
    ...
    pub fn with_config(config: SolverConfig) -> Self {
        ...
        if let Ok(v) = std::env::var("NIXIE_CHRONO_REUSE") {
            config.chrono_reuse = v == "1";
        }
        if let Ok(v) = std::env::var("NIXIE_CHRONO_REUSE_AFTER")
    ...
    /// saved phase.  The random-polarity perturbation is an Nixie extension
    ...
    pub fn solve(&mut self) -> SolverResult {
        ...
        if self.config.enable_sbva || std::env::var("NIXIE_SBVA").as_deref() == Ok("1") {
        ...
        if self.config.enable_xor_reasoning || std::env::var("NIXIE_XOR").as_deref() == Ok("1") {
        ...
        if std::env::var("NIXIE_XORPROBE").is_ok() {
        ...
        // `NIXIE_XORSEARCH=1`): the CryptoMiniSat-shaped integration — the
        ...
        if std::env::var("NIXIE_XORSEARCH").is_ok() {
        ...
        // collapsing to 0 conflicts.  Default off; `NIXIE_PREWALK=<flips>`
        // is the A/B knob.
        if let Ok(v) = std::env::var("NIXIE_PREWALK")
    ...
    /// Get statistics
    #[must_use]
    /// Search-state memory composition (diagnostics: `NIXIE_MEM_STATS` in
    ...
    /// `NIXIE_INPROC_TRACE`-style tooling.
    #[doc(hidden)]
    ...
    /// Shrink-study accumulator (see `conflict.rs`; `NIXIE_SHRINK_TRACE=1`).
    #[must_use]
    ...
    /// see `NIXIE_SHRINK_TRACE` in `conflict.rs`).
    #[doc(hidden)]
