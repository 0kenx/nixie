...
use nixie_sat::{LBool, Lit, Solver, SolverResult};
...
fn model_satisfies(solver: &Solver, clauses: &[Vec<i32>]) -> bool {
    for clause in clauses {
        ...
        for &dlit in clause {
            ...
            let val = solver.model_value(nixie_sat::Var::new(var_idx as u32));
...
fn binary_chain_forced_false_is_sat_with_valid_model() {
    ...
    for i in 0..6 {
        assert_eq!(
            solver.model_value(nixie_sat::Var::new(i)),
            LBool::False,
            "x{} must be false",
            i + 1
        );
