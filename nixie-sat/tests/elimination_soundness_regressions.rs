...
use nixie_sat::{DimacsParser, Lit, Solver, SolverConfig, SolverResult};
...
fn eliminated_variable_is_never_a_fallback_decision() {
    ...
    assert_eq!(solver.solve(), SolverResult::Sat);
    let b_ok = solver.model_value(b) == nixie_sat::LBool::True;
    let c_ok = solver.model_value(c) == nixie_sat::LBool::True;
    assert!(
        b_ok || c_ok,
...
fn elimination_refused_under_lrat_terminates() {
    ...
    let mut solver = Solver::with_config(solver_cfg);
    solver
        .enable_lrat_proof("/tmp/nixie_sat_elim_lrat_regression.lrat")
    ...
    let _ = std::fs::remove_file("/tmp/nixie_sat_elim_lrat_regression.lrat");
