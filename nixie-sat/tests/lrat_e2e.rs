...
/// is found, lazily compile the vendored `nixie-sat/tools/lrat-check.c` so the
...
fn build_vendored_lrat_check() -> Option<PathBuf> {
    ...
    let bin = std::env::temp_dir().join("nixie-lrat-check-bin");
    ...
    // Compile into a unique per-process staging file, then atomically rename
    // it onto `bin`.  nextest runs every test as its own process, so several
    // processes can reach this point concurrently; compiling straight to the
    // shared path lets one process exec a half-written binary (a real flake
    // under parallel runs).  The rename is atomic, so any reader of `bin`
    // always sees a complete executable, and a process exec-ing it while
    // another renames keeps running on the old inode.
    let staging = std::env::temp_dir().join(format!(
        "nixie-lrat-check-bin.staging-{}",
        std::process::id()
    ));
    ...
    if std::fs::rename(&staging, &bin).is_ok() {
        Some(bin)
    } else {
        // Same-directory rename should not fail; if it somehow does, the
        // fully-built staging binary is still a usable checker.
        Some(staging)
    }
}
...
fn assert_verified(nvars: usize, clauses: &[Vec<i32>], binary: bool) {
    ...
    let dir = std::env::temp_dir().join(format!("nixie_lrat_e2e_{}_{seq}", std::process::id()));
...
fn lrat_shrink_keeps_plain_search_shape() {
    ...
    let dir = std::env::temp_dir().join(format!("nixie_lrat_shrink_{}", std::process::id()));
    let _ = fs::create_dir_all(&dir);
    let cnf = dir.join("in.cnf");
...
fn lrat_bve_unit_resolvent_provenance() {
    ...
    let dir = std::env::temp_dir().join(format!("nixie_lrat_bveunit_{}", std::process::id()));
    let _ = fs::create_dir_all(&dir);
    let cnf = dir.join("in.cnf");
    let lrat = dir.join("out.lrat");
    let mut f = fs::File::create(&cnf).unwrap();
...
fn lrat_subsume_round_strengthen_under_proof() {
    ...
    let dir = std::env::temp_dir().join(format!("nixie_lrat_subsume_{}", std::process::id()));
    let _ = fs::create_dir_all(&dir);
    let cnf = dir.join("in.cnf");
    let lrat = dir.join("out.lrat");
    let mut f = fs::File::create(&cnf).unwrap();
