...
//!
//! `nixie-sat` used to publish `cuda`/`opencl`/`vulkan` Cargo features whose
...
fn cargo_toml_does_not_publish_fake_gpu_backend_features() {
    for banned in ["cuda", "opencl", "vulkan"] {
        ...
        assert!(
            !declares_feature,
            "nixie-sat/Cargo.toml must not declare a `{banned}` Cargo feature: \
...
fn no_gpu_backend_types_are_publicly_exported() {
    ...
    assert!(
        ...
        "nixie-sat/src/lib.rs must not reference a `gpu` module"
    );
    assert!(
        !lib_rs.to_lowercase().contains("gpubackend"),
        "nixie-sat/src/lib.rs must not re-export GPU accelerator types"
    );
}
