...
//! proof, and check it with `nixie_proof::lrat_check`'s pure-Rust LRAT
...
fn unique_lrat_path(tag: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "nixie_sat_pr26_lrat_{tag}_{}_{nanos}.lrat",
        std::process::id()
    ))
}
...
fn test_pr26_lrat_dimacs_fixture_proof_verifies_via_files() {
    ...
    // CNF and LRAT *files* via `nixie_proof::lrat_check::check_lrat_files` –
    ...
    let report =
        nixie_proof::lrat_check::check_lrat_files(&cnf_path, &lrat_path).expect("check files");
    assert!(report.verified, "failure: {:?}", report.failure);
...
fn test_pr26_lrat_writer_is_inert_after_unsat_finalization() {
    ...
    // checker stops reading at that line (see `nixie_proof::lrat_check`), so
