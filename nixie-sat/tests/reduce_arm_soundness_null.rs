...
//! returned Unsat under `NIXIE_CADICAL_REDUCE_NULL=1` / `NIXIE_REDUCE_BY_USED=1`
//! / `NIXIE_REDUCE_ADAPT=1` before the fix (2026-09-02).
//!
...
use nixie_sat::{ConfigPreset, DimacsParser, Solver, SolverResult};
...
fn random_null_reduce_never_yields_wrong_unsat_on_constraints_17() {
    ...
    unsafe {
        std::env::set_var("NIXIE_CADICAL_REDUCE_NULL", "1");
    }
