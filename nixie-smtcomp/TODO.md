...
- [x] Caching of parsing results (planned 2026-04-15, completed 2026-04-15)
  ...
  - **Files:** `nixie-smtcomp/src/loader.rs`, `nixie-smtcomp/src/lib.rs` (re-export).
...
- [x] Machine learning-based difficulty prediction (planned 2026-05-04, completed 2026-05-05)
  ...
  - **Files:** `nixie-smtcomp/src/predictor/{mod,features,dataset,models,linear,knn,tree,class,persistence,report}.rs` (new); `nixie-smtcomp/src/lib.rs`, `nixie-smtcomp/src/parallel.rs` (modified); 6 new integration test files.
...
- [x] Automatic logic detection from formulas (planned 2026-04-15, completed 2026-04-15)
  ...
  - **Design:** New `nixie-smtcomp/src/logic_detector.rs` with `LogicDetector` visitor collecting `TheoryBits`. Bits→logic table (e.g., `UF | LIA → "UFLIA"`, `ARR | BV → "QF_ABV"`), fallback `"ALL"` if unrepresentable. Call path integrated into `loader.rs` when logic unknown. Subagent to verify what AST type is available in the loader path (term AST vs. raw text) and pick the correct layer.
  - **Files:** `nixie-smtcomp/src/logic_detector.rs` (new), `nixie-smtcomp/src/lib.rs`, `nixie-smtcomp/src/loader.rs`.
...
- [x] PDF report generation (planned 2026-04-15, completed 2026-04-15)
  ...
  - **Design:** New `nixie-smtcomp/src/pdf_report.rs` mirroring the `HtmlReport` API (`PdfReport::from_runs(...)` → `write(path)`). Use a latest Pure Rust PDF crate; subagent picks (`printpdf` or `pdf-writer`) after checking crates.io latest versions. Simple tabular layout, default font only.
  - **Files:** `nixie-smtcomp/src/pdf_report.rs` (new), `nixie-smtcomp/Cargo.toml` (feature + dep via workspace), root `Cargo.toml` (workspace dep entry), `nixie-smtcomp/src/lib.rs` (feature-gated re-export).
...
- [x] Docker container generation for reproducible runs (planned 2026-04-15, completed 2026-04-15)
  ...
  - **Design:** Multi-stage Dockerfile at `nixie-smtcomp/Dockerfile`. `.dockerignore` excludes target/, .git/. Entrypoint script at `nixie-smtcomp/docker/entrypoint.sh` forwards args to the binary. Rust toolchain version pinned via build arg (default: current stable).
  - **Files:** `nixie-smtcomp/Dockerfile` (new), `nixie-smtcomp/.dockerignore` (new), `nixie-smtcomp/docker/entrypoint.sh` (new).
