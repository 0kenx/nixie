...
//! JSON-encoded [`nixie_smtcomp::ParallelProgress`] events in real time.

#[cfg(not(feature = "ws-progress"))]
fn main() {
    eprintln!(
        "This binary requires the `ws-progress` feature.\n\
         Rebuild with: cargo build -p nixie-smtcomp --features ws-progress --bin ws_server"
    );
    std::process::exit(1);
}
    ...
    fn parse_args() -> Result<Args, String> {
        ...
        while i < raw.len() {
            match raw[i].as_str() {
                ...
                "--help" | "-h" => {
                    println!(
                        "ws_server -- Nixie WebSocket progress server\n\n\
                         USAGE:\n\
    ...
    pub async fn run() {
        ...
        let solved = results
            ...
            .filter(|r| {
                matches!(
                    r.status,
                    nixie_smtcomp::BenchmarkStatus::Sat | nixie_smtcomp::BenchmarkStatus::Unsat
                )
            })
            .count();
