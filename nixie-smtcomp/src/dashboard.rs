...
(function() {
    // ======== Search / filter ========
    var searchInput = document.getElementById('nixie-search');
...
/// JavaScript template for WebSocket live-update. The placeholder __NIXIE_WS_URL__ is replaced
/// with the actual URL at generation time so that Rust format! braces are not involved.
const JS_WS_TEMPLATE: &str = r#"
<script>
(function() {
    var ws = new WebSocket("__NIXIE_WS_URL__");
    ...
    ws.onerror = function() { console.warn('Nixie dashboard WebSocket error'); };
})();
</script>
"#;
...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="search-bar">
                <input id="nixie-search" type="text" placeholder="Filter by name or logic...">
            </div>
    ...
    <footer>
        Generated by Nixie SMT-COMP Infrastructure
    </footer>
    ...
    new Chart(document.getElementById('logicChart'), {{
        ...
        options: {{
            ...
            onClick: function(evt, elements) {{
                if (elements && elements.length > 0 && window.nixieSetLogicFilter) {{
                    var el = elements[0];
                    ...
                    if (logicName) window.nixieSetLogicFilter(logicName);
                    if (statusName) window.nixieSetStatusFilter(statusName);
                }}
            }},
...
"##,
        ...
        if let Some(ref url) = self.config.ws_url {
            let ws_js = JS_WS_TEMPLATE.replace("__NIXIE_WS_URL__", url);
            html.push_str(&ws_js);
        }
