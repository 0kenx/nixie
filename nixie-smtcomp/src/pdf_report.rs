...
//! use nixie_smtcomp::pdf_report::{PdfReportConfig, PdfReportGenerator};
//! use nixie_smtcomp::SingleResult;
//!
//! let results: Vec<SingleResult> = Vec::new();
//! let config = PdfReportConfig::new("Nixie Benchmark Run");
//! let generator = PdfReportGenerator::new(config);
//! generator
//!     .write_to_file(&results, "/tmp/nixie_report.pdf")
...
mod tests {
    ...
    fn test_pdf_report_minimal() {
        let path = std::env::temp_dir().join("nixie_pdf_test_minimal.pdf");
    ...
    fn test_pdf_report_multi_logic() {
        let path = std::env::temp_dir().join("nixie_pdf_test_multi_logic.pdf");
        let _ = fs::remove_file(&path);
        ...
        let config = PdfReportConfig::new("Nixie Multi-Logic PDF Test")
            .with_solver("Nixie-test", env!("CARGO_PKG_VERSION"))
    ...
    fn test_pdf_report_generator_write_to_file() {
        let path = std::env::temp_dir().join("nixie_pdf_test_generator_api.pdf");
        let _ = fs::remove_file(&path);
