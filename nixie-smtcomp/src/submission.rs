...
/// All SMT-COMP 2026 divisions supported by Nixie.
...
pub struct SubmissionConfig {
    ...
    /// [`SubmissionConfig::default_nixie_2026`].
...
impl SubmissionConfig {
    ...
    /// Create a default Nixie submission config for all 2026 divisions and tracks.
    pub fn default_nixie_2026() -> Self {
        ...
        Self::new("Nixie", env!("CARGO_PKG_VERSION"), contact)
            .with_description(
                "Nixie: A next-generation SMT solver written in pure, safe Rust. \
            ...
            .with_starexec_config("nixie-smtcomp2026")
...
mod tests {
    ...
    fn test_generate_submission_package() {
        let output_dir = std::env::temp_dir().join("nixie_smtcomp_test_pkg");
        let _ = fs::remove_dir_all(&output_dir);

        let contact = ContactInfo::new("Test User", "test@example.com", "Test Org")
            .with_url("https://example.com");
        let config = SubmissionConfig::new("Nixie", "0.2.2", contact)
            .with_division("QF_LIA")
            .with_division("QF_BV");
    ...
    fn test_generate_package_missing_field() {
        let output_dir = std::env::temp_dir().join("nixie_smtcomp_test_empty");
        let contact = ContactInfo::new("Test", "t@t.com", "Org");
    ...
    fn test_default_config_uses_smtcomp_binary() {
        let cfg = SubmissionConfig::default_nixie_2026();
    ...
    fn test_generate_emits_per_track_scripts() {
        let dir = std::env::temp_dir().join(format!("nixie_track_scripts_{}", std::process::id()));
        let cfg = SubmissionConfig::default_nixie_2026();
