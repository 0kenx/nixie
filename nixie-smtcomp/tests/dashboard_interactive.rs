...
use nixie_smtcomp::benchmark::{BenchmarkStatus, SingleResult};
use nixie_smtcomp::dashboard::{DashboardConfig, DashboardGenerator};
use nixie_smtcomp::loader::{BenchmarkMeta, ExpectedStatus};
...
fn test_dashboard_renders_search_input() {
    ...
    assert!(
        html.contains(r#"id="nixie-search""#),
        "HTML should contain the search input with id='nixie-search'"
    );
}
