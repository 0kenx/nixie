//! Integration tests for nixie-smtcomp
//!
//! These tests verify the integration of various components with
//! real or simulated benchmark data.

use nixie_smtcomp::benchmark::{BenchmarkStatus, RunSummary, Runner, RunnerConfig, SingleResult};
use nixie_smtcomp::filtering::{
    ExpectedStatusFilter, FilterCriteria, ResultFilterCriteria, filter_benchmarks, filter_results,
};
use nixie_smtcomp::loader::{Benchmark, BenchmarkMeta, ExpectedStatus};
use nixie_smtcomp::parallel::{ParallelConfig, ParallelRunner};
use nixie_smtcomp::plotting::SvgPlot;
use nixie_smtcomp::regression::RegressionDetector;
use nixie_smtcomp::reporter::Reporter;
use nixie_smtcomp::resumption::{CheckpointConfig, ResultLoader, ResultSaver};
use nixie_smtcomp::sampling::{Sampler, SamplingConfig, SamplingStrategy};
use nixie_smtcomp::starexec::StarExecWriter;
use nixie_smtcomp::statistics::{SolverComparison, Statistics, cactus_data, scatter_data};
use nixie_smtcomp::virtual_best::VirtualBestSolver;
...
fn test_starexec_format() {
    ...
    let mut writer = StarExecWriter::new("Nixie", "default");
...
fn test_html_report_generation() {
    use nixie_smtcomp::html_report::{HtmlReportConfig, HtmlReportGenerator};
...
fn test_dashboard_generation() {
    use nixie_smtcomp::dashboard::{DashboardConfig, DashboardGenerator};
...
fn test_ci_integration() {
    use nixie_smtcomp::ci_integration::{
        CiConfig, CiConfigGenerator, CiSystem, generate_junit_xml,
    };
...
fn test_memory_limits() {
    use nixie_smtcomp::memory::{MemoryLimit, MemoryMonitor, MemoryUsage};
...
fn test_model_verification() {
    use nixie_smtcomp::model_verify::Model;
...
fn test_end_to_end_benchmark_run() {
    ...
    plot.add_results("Nixie", &results);
    let svg = plot.to_svg().unwrap();
