...
#[cfg(test)]
mod tests {
    use nixie_smtcomp::submission::{SubmissionConfig, Track, generate_submission_package};
    use std::fs;

    fn temp_dir(suffix: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!("nixie_e2e_{}_{}", suffix, std::process::id()))
    }

    #[test]
    fn test_submission_module_accessible() {
        let _ = SubmissionConfig::default_nixie_2026();
    }
    ...
    fn test_generate_package_conf_contains_solver_name() {
        ...
        assert!(
            conf.contains("Nixie") || conf.contains("nixie"),
