...
The nixie-solver crate closed the last of the parity gap: **168/168 Correct,
...
The nixie-solver crate contributed to matching z3 on all 88 benchmarks of the
...
## Dependencies
- **nixie-core**: AST, SMT-LIB2 parser, tactic framework
- **nixie-sat**: CDCL SAT solver
- **nixie-theories**: Theory solvers (EUF, LRA, LIA, BV, Arrays, Strings, FP)

## Provides (enables other crates)
- **nixie-spacer**: SMT solving for PDR queries
- **nixie-opt**: OMT solving for optimization
- **nixie-wasm/nixie-cli**: Main solver API
...
- [x] TheoryCombination trait moved to nixie-theories (proper architecture)
...
Note: These features are implemented in `nixie-sat` and exposed through `SolverConfig` in nixie-solver.
...
- [x] `Context::eval_in_model` – new public method; evaluates a TermId against the current model (clones Model, calls eval over ctx.terms). Added for nixie-spacer BMC counterexample extraction.
