...
/// `nixie_core::model` types into the public API of this file.
...
impl Context {
    ...
    /// [`nixie_proof::logging::ProofLogger`] at that path, writes a structural
    ...
    ///
    /// Delegates to [`nixie_proof::replay::ProofReplayer::replay_from_file`].
    ///
    /// # Errors
    ///
    ...
    pub fn verify_proof_log(
        path: &Path,
    ) -> std::result::Result<nixie_proof::replay::VerificationResult, nixie_proof::replay::ProofError>
    {
        nixie_proof::replay::ProofReplayer::replay_from_file(path)
    }
    ...
    fn write_proof_log(
        ...
        path: &Path,
        result: SolverResult,
    ) -> std::result::Result<(), nixie_proof::logging::LoggingError> {
        use nixie_proof::logging::ProofLogger;
        use nixie_proof::proof::{ProofNodeId, ProofStep};
    ...
    /// an `nixie-solver` core change.  They are intentionally left as no-ops rather
            ...
            "reason-unknown" => {
                ...
                match self.last_result {
                    Some(SolverResult::Unknown) => match self.certification_failure() {
                        Some(reason) => format!(
                            "(:reason-unknown {})",
                            nixie_core::smtlib::format_string_literal(reason)
                        ),
    ...
    pub(crate) fn check_with_assumptions_raw(
        &mut self,
        assumptions: &[nixie_core::ast::TermId],
        ...
        for cmd in commands {
            ...
            match cmd {
                ...
                Command::DefineSort(name, params, sort_expr) => {
                    ...
                    // `nixie_core`'s `Parser::parse_sort_name`), so there is
                ...
                Command::DefineFun(name, params, ret_sort, body) => {
                    ...
                    if params.is_empty() {
                        ...
                        // time (see `nixie_core`'s defined-function handling
