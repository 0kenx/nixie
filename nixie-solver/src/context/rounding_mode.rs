...
//!
//! `nixie_core::sort::SortKind::RoundingMode` gives rounding modes a sort of
...
//! user. These axioms are Nixie's internal encoding of a built-in sort, not
//! something the user wrote, so they go straight to `Solver::assert` and stay
//! out of that list.

use super::Context;
use nixie_core::ast::{RoundingMode, TermId};
