...
impl Context {
    ...
    fn malformed_sort_error(expr: &str) -> NixieError {
        NixieError::Unsupported(format!("unsupported or malformed sort expression: {expr}"))
    }

    /// Resolve a sort-expression string into a `SortId`.
    ///
    /// The strings handled here are exactly the ones `nixie_core`'s
    ...
    /// `(Array dom rng)` yields [`NixieError::Unsupported`].  It used to
...
mod tests {
    ...
    fn test_malformed_compound_is_error_not_bool() {
        ...
        for malformed in [
            ...
            match got {
                Err(NixieError::Unsupported(msg)) => {
                    assert!(
