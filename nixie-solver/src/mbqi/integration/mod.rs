...
impl MBQIIntegration {
    ...
    pub fn add_quantifier(&mut self, term: TermId, manager: &TermManager) {
        ...
        match &t.kind {
            nixie_core::ast::TermKind::Forall { vars, body, .. } => {
            ...
            nixie_core::ast::TermKind::Exists { vars, body, .. } => {
                let bound_vars: SmallVec<[(Spur, SortId); 4]> = vars.iter().copied().collect();
                self.quantifiers
    ...
    pub fn collect_ground_terms(&mut self, term: TermId, manager: &TermManager) {
        ...
        let subterms = nixie_core::ast::traversal::collect_subterms(term, manager);
        ...
        for &sub in &subterms {
            ...
            let ground = if matches!(t.kind, TermKind::Var(_)) {
                ...
                nixie_core::ast::traversal::get_children(&t.kind)
                    .iter()
...
struct InstantiationKey {
    quantifier: TermId,
    binding: Vec<(nixie_core::interner::Spur, TermId)>,
}
