...
//! `nixie-sat`'s pre-existing [`nixie_sat::BranchingHeuristic`] extension
//! point (added for `nixie-ml`; nothing here required touching `nixie-sat`).
//!
//! # Why this is a config knob, not a default
//!
//! `nixie-sat`'s `pick_branch_var` builds its candidate list – every
...
//! all (see `nixie-sat/src/solver/decide.rs`). That scan happens on every
...
//! field is `pub(super)` inside `nixie-sat`, so a later `set_config` call on
...
use nixie_sat::{BoxedBranchingHeuristic, BranchingHeuristic, Var};
...
/// `nixie_sat::SolverConfig::external_branching`.
