...
impl Solver {
    ...
    ///
    /// RESTRICTION: only the fresh `ite`-result constants (`__nixie_ite_*`)
    ...
    /// restart).  nixie's `TheoryCallback` cannot encode mid-search, so the
    ...
    pub(super) fn pre_encode_care_graph_atoms(&mut self, manager: &mut TermManager) {
        ...
        // shared-terms set; nixie has no purification so the interface is the
        ...
        // nixie has none, so a large shared interface yields O(n^2) care atoms
        ...
        'outer: for i in 0..shared.len() {
            ...
            for &b in shared.iter().skip(i + 1) {
                ...
                if !matches!(
                    ...
                    nixie_theories::arithmetic::ArithEqualityStatus::Unknown
                ) {
                    continue;
                }
    ...
    pub(super) fn bump_finite_domain_enumerations(&mut self, manager: &TermManager) {
        use rustc_hash::FxHashSet;
        let mut atoms: Vec<nixie_sat::Var> = Vec::new();
    ...
    fn collect_quantifier_funcs(&mut self, body: TermId, manager: &TermManager) {
        for st in nixie_core::ast::collect_subterms(body, manager) {
    ...
    pub(super) fn eliminate_nonbool_ite(
        ...
        for st in collect_ground_subterms(term, manager) {
            ...
            if matches!(t.kind, TermKind::Ite(..)) && needs_ite_elimination(t.sort, manager) {
                let v = manager.mk_var(&format!("__nixie_ite_{}", st.0), t.sort);
    ...
    pub(super) fn abstract_compound_bool_args(
        ...
        use nixie_core::ast::collect_subterms;
        use rustc_hash::FxHashMap;
        ...
        for arg in to_abstract {
            ...
            let v = manager.mk_var(&format!("__nixie_boolarg_{}", arg.0), bool_sort);
            map.insert(arg, v);
        }
    ...
    pub(super) fn purify_numeric_uf_args(
        ...
        for arg in to_abstract {
            ...
            let Some(at) = manager.get(arg) else { continue };
            let v = manager.mk_var(&format!("__nixie_numarg_{}", arg.0), at.sort);
            map.insert(arg, v);
        }
        let mut side: Vec<TermId> = Vec::with_capacity(map.len());
        for (arg, v) in &map {
            ...
            TermKind::Eq(lhs, rhs) => {
                ...
                if lhs_is_array {
                    ...
                    if let Some(domain) = array_domain_sort {
                        ...
                        let k = manager.mk_var(&format!("!nixie!ext!{lo}!{hi}"), domain);
...
pub(super) fn blast_bv_term(
    bv: &mut nixie_theories::bv::BvSolver,
    root: TermId,
    manager: &TermManager,
    encoded: &mut rustc_hash::FxHashSet<TermId>,
) {
    for st in nixie_core::ast::traversal::collect_subterms(root, manager) {
