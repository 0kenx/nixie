...
///
/// There is no `mk_match` smart constructor in `nixie-core`, and `MatchCase`
/// is not part of its public API surface (it is not re-exported from
/// `nixie_core::ast`, and the module that defines it is private) -- so from
...
/// possible from outside `nixie-core` without changing that crate.
#[test]
