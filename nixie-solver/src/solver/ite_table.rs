...
impl Solver {
    ...
    pub(super) fn flatten_eq_ite_tables(
        ...
        for (i, m) in maximal.iter().enumerate() {
            ...
            let r = manager.mk_var(&format!("__nixie_tbl_{}_{}", m.root.0, i), sort);
    ...
    pub(super) fn eager_table_index_case_split(&mut self, manager: &mut TermManager) {
        ...
        let dbg = std::env::var("NIXIE_DBG_ITE").is_ok();
        ...
        let mut domain_priority_scored: Vec<(i64, i64, nixie_sat::Var)> = Vec::new();
