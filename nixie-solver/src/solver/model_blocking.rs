...
//! # Scoping
//!
//! The clauses are added with [`nixie_sat::Solver::add_clause`], so they live
...
impl Solver {
    ...
    ///
    /// Every call site that turns an `nixie_sat::SatResult::Unsat` into an
    ...
    pub(super) fn refuted_model_projection(&self) -> Vec<Lit> {
        ...
        for idx in 0..self.var_to_term.len() {
            ...
            // `nixie_sat::Var` stores as a `u32`; a length past `u32::MAX` is
    ...
    ///
    /// `false` from [`nixie_sat::Solver::add_clause`] means the clause was
