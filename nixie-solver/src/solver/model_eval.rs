...
/// The model certificate's tri-state (see
/// [`Solver::model_certificate_status`]).
pub(super) enum CertStatus {
    /// Every assertion verified true under the published model.
    Pass,
    /// Some assertion concretely FALSE: the candidate violates its own
    /// assertions and may be blocked-and-retried.
    Refuted,
    /// A conjunct the evaluator cannot decide: the honest decline.
    Undecided,
}
...
impl Solver {
    ...
    pub(super) fn model_certifies_assertions(&self, manager: &TermManager) -> bool {
        matches!(self.model_certificate_status(manager), CertStatus::Pass)
    }

    /// The certificate's TRI-STATE: [`CertStatus::Pass`] (every assertion
    /// verified true under the published model), [`CertStatus::Refuted`]
    /// (some assertion concretely FALSE — the candidate violates its own
    /// assertions; the J5 gate may block-and-retry it),
    /// [`CertStatus::Undecided`] (a conjunct the evaluator cannot decide —
    /// the honest decline, never blocked).
    pub(super) fn model_certificate_status(&self, manager: &TermManager) -> CertStatus {
        let Some(model) = self.model.as_ref() else {
        ...
        *self.certify_delta0.borrow_mut() = self.arith.delta_instantiation_exact();
        ...
        // certificate (it is exactly how a holed model still passed in
        // the general path's variant of this gate).
                ...
                match self.eval_in_model_outcome(conj, model, manager, 0) {
                    ...
                    other => {
                        ...
                        *self.certify_delta0.borrow_mut() = saved_d0;
                        return match other {
                            EvalOutcome::Value(EvalVal::Bool(false)) => CertStatus::Refuted,
                            _ => CertStatus::Undecided,
                        };
                    }
                }
            }
    ...
    /// most arguments arrive as `__nixie_numarg_N` proxy vars, whose
        ...
        match &t.kind {
            ...
            TermKind::Var(_) => Opened::Done({
                ...
                if sort == manager.sorts.int_sort || sort == manager.sorts.real_sort {
                    ...
                    // variables (the `__nixie_*`/`$p*` names), because the
