...
impl Solver {
    ...
    pub(super) fn dispatch_nla_relaxation(
        ...
        use nixie_theories::arithmetic::nla::{self, NlaConfig, NlaVerdict};
        use nixie_theories::nl_eval::holds_under;
    ...
    fn adopt_interpretation(
        &mut self,
        witness: nixie_theories::nl_eval::Interpretation,
        manager: &mut TermManager,
    ) -> bool {
        ...
        while let Some(t) = stack.pop() {
            ...
            match &td.kind {
                ...
                | TermKind::Neg(_) => {
                    ...
                    stack.extend(nixie_core::ast::traversal::get_children(&td.kind));
                }
                _ => {
