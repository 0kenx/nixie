...
//! `nixie_sat::Solver::num_original_clauses`, never `num_clauses()` minus
...
///    (`nixie_sat::Solver::check_hyper_binary_resolution`) added clauses to the
...
/// `cargo nextest` parallel – `nixie` uses `FxHashMap` and deterministic CDCL,
...
/// variables, which `nixie_sat::Solver::add_clause` (no duplicate detection)
...
/// # The failure mode
///
/// `nixie_sat::Solver` keeps two ledgers beside the clause database:
