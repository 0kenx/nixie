//! Static formula features – the nixie port of Z3's `ast/static_features`.
//!
//! # What this is
//!
...
//!
//! nixie used to route purely on the logic string:
//!
//! ```ignore
...
//! strictly more correct than Z3's structural test on nixie's richer AST:
//!
//! * **Diff shape is computed from a linear form, not from raw `Add`/`Mul`
//!   constructor patterns.**  Z3's `is_diff_atom` recognises `(+ x (* -1 y))`
//!   literally; nixie keeps `Sub`, `Neg`, and variadic `Add`/`Mul`, so a literal
...
//!   away before feature collection).  nixie keeps `Lt`/`Gt` as distinct kinds
...
/// [`StaticFeatures`] first.  nixie always collects features and lets the logic
...
impl StaticFeatures {
    ...
    pub fn inferred_logic(&self) -> Option<&'static str> {
        ...
        // nixie's MBQI/e-matching handles those uniformly, so we do not pretend
    ...
    fn note_sort(&mut self, manager: &TermManager, sort: SortId) {
        ...
        match s.kind {
            ...
            SortKind::Set(_) | SortKind::Bag(_) => self.has_set = true,
...
fn sort_class(manager: &TermManager, sort: SortId) -> SortClass {
    ...
    match s.kind {
        ...
        SortKind::Set(_) | SortKind::Bag(_) => SortClass::Set,
