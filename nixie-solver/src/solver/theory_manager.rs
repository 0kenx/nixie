...
use nixie_theories::{EqualityNotification, Theory, TheoryCombination};
...
/// Whether theory-conflict tracing is enabled (`NIXIE_TRACE_DECISIONS`, shared
...
/// SAT-side `nixie-conflict` lines (which report the detection *point* but not
...
fn theory_trace_enabled() -> bool {
    ...
    *FLAG.get_or_init(|| {
        std::env::var("NIXIE_TRACE_DECISIONS")
            .is_ok_and(|v| !v.is_empty() && v != "0" && !v.eq_ignore_ascii_case("false"))
    })
}

/// Incremental arithmetic bound-propagation mode, set from `NIXIE_BOUND_PROP`
...
pub(crate) fn arith_bound_prop_mode() -> BoundPropMode {
    ...
    *FLAG.get_or_init(|| {
        match std::env::var("NIXIE_BOUND_PROP") {
            Ok(v)
...
fn feed_dl_atom_inc(
    dl: &mut nixie_theories::DiffLogicSolver,
    dla: &DlAtom,
    pol: bool,
    origin: TermId,
) -> nixie_theories::DiffLogicResult {
    use ArithConstraintType::*;
    use nixie_theories::DiffLogicResult;
...
fn is_plain_numeric_term(td: &nixie_core::ast::Term) -> bool {
    matches!(
        td.kind,
...
fn direct_const_value(manager: &nixie_core::ast::TermManager, t: TermId) -> Option<Rational64> {
...
pub(crate) struct TheoryManager<'a> {
    ...
    bv: &'a mut BvSolver,
    diff: &'a mut nixie_theories::DiffLogicSolver,
    ...
    /// `(+ x 1)` -> `(+ x __nixie_numarg)`, which destroys difference-logic
    ...
    zero_term: TermId,
    /// Fresh `ite`-result constants (`__nixie_ite_*`) axiomatized against
    ...
    /// answers `Unknown`.
    #[cfg(feature = "std")]
    deadline: Option<nixie_time::Instant>,
...
impl<'a> TheoryManager<'a> {
    ...
    pub(crate) fn new(
        ...
        let deadline = if timeout_ms > 0 {
            nixie_time::Instant::now().checked_add(core::time::Duration::from_millis(timeout_ms))
        } else {
            None
        };
        #[cfg(not(feature = "std"))]
    ...
    fn timed_out(&self) -> bool {
        ...
        {
            match self.deadline {
                Some(d) => nixie_time::Instant::now() >= d,
                None => false,
            }
        }
        #[cfg(not(feature = "std"))]
    ...
    fn model_based_combination(&mut self) -> TheoryCheckResult {
        ...
        if asserted_any {
            use nixie_theories::Theory;
            if let Ok(nixie_theories::TheoryCheckResult::Unsat(conflict_terms)) = self.arith.check()
            {
                return self.conflict_from_terms(&conflict_terms);
            }
        }
    ...
    fn arrange_model_equal_pairs(&mut self) {
        ...
        let mut by_val: FxHashMap<(Rational64, nixie_core::SortId), Vec<TermId>> =
            FxHashMap::default();
    ...
    fn arrangement_cross_check_arith(&mut self) -> bool {
        ...
        let unsat = matches!(
            self.arith.check(),
            Ok(nixie_theories::TheoryCheckResult::Unsat(_))
        );
        self.arith.pop();
        unsat
    }
    ...
    fn derive_arith_bound_propagations(
        ...
        use nixie_theories::arithmetic::DeltaRational;
    ...
    /// If `t` is a `__nixie_numarg` proxy (a fresh variable that
    ...
    /// Folds `__nixie_numarg` proxies and bare constants into the atom's RHS so
    ...
    /// (`__nixie_numarg`, recoverable via `numarg_proxies`) or a bare
    ...
    fn dl_watch_of(&self, dla: &DlAtom, c: &Rational64) -> Option<DlWatch> {
        ...
        let (x, y) = (dla.x, dla.y);
        let one = Rational64::from_integer(1);
        let k_of = |r: &Rational64| nixie_theories::DiffLogicSolver::dense_fit(r);
        if dla.is_eq {
            let ck = k_of(c)?;
            return Some(DlWatch {
                watch_s: y,
        ...
        for &(src_term, dst_term, w) in &feed.edges {
            ...
            match outcome {
                Some(DlAssert::Conflict(reason)) => {
                    #[cfg(feature = "std")]
                    if theory_trace_enabled() {
                        eprintln!("nixie-tconf\tdiff-dense");
                    }
            ...
            Constraint::Eq(lhs, rhs) => {
                if is_positive {
                    ...
                    if let Some(parsed) = self.var_to_parsed_arith.get(&var).cloned() {
                        ...
                        if let Ok(nixie_theories::TheoryCheckResult::Unsat(conflict_terms)) =
...
impl TheoryCallback for TheoryManager<'_> {
    ...
    /// (see `TheoryCallback::is_real_theory` in nixie-sat for the soundness
    /// argument).
    ...
    fn on_assignment(&mut self, lit: Lit) -> TheoryCheckResult {
        ...
        // analogue).  Env-gated (`NIXIE_BOUND_PROP`); validated sound before
    ...
    fn final_check(&mut self) -> TheoryCheckResult {
        ...
        if !self.var_to_parsed_arith.is_empty()
            && let nixie_theories::DiffLogicResult::Conflict(cycle_terms) = self.diff.check()
        {
