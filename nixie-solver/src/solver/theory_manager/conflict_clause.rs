...
impl TheoryManager<'_> {
    ...
    ///
    /// `analyze_theory_conflict` (in `nixie-sat`) requires every literal of the
    ...
    ///
    /// `analyze_theory_conflict` (in `nixie-sat`) requires every literal of a
...
mod tests {
    ...
    struct Fixture {
        ...
        arith: ArithSolver,
        bv: BvSolver,
        diff: nixie_theories::DiffLogicSolver,
    ...
    fn an_unjustifiable_conflict_aborts_instead_of_refuting() {
        use nixie_sat::TheoryCheckResult;
