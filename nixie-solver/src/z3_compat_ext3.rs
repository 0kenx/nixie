...
//!
//! [`TermManager::mk_forall_with_patterns`]: nixie_core::ast::TermManager::mk_forall_with_patterns
//! [`TermManager::mk_exists_with_patterns`]: nixie_core::ast::TermManager::mk_exists_with_patterns
...
///
/// This collapses Nixie's richer [`SortKind`] into the
...
impl Z3Context {
    ...
    ///   (Nixie has no separate bound-variable representation -- a
    ...
    /// `nixie_core::ast::traversal::map_terms` retired its own
...
/// instantiation of a quantifier.  In Nixie a pattern is materialised as the
...
mod tests {
    ...
    /// `nixie-core`'s `ast/manager/query/tests.rs`: a stack overflow aborts
    ...
    fn substitute_survives_deep_not_chain_on_tiny_stack() {
        ...
        let (reached_leaf, old_leaf_gone) = run_on_small_stack(|| {
            ...
            // `TermManager::intern` is `pub(crate)` in nixie-core and not
