...
//! z3-certified: `F ∧ (= pc0 -1)` alone is UNSAT while nixie's model pinned
...
/// *entail* `a = b`, so congruence forces equal results; nixie and z3 agree
/// on that too.)
#[test]
...
/// ```
///
/// (equivalently `cargo test -p nixie-solver --test
...
#[test]
#[ignore = "known-slow debug canary (~330 s debug / ~25 s release); run explicitly before landing: cargo nextest run -p nixie-solver --run-ignored only -E 'test(pete_cxs_bp)' (see the doc comment above)"]
