...
//!   push/solve/pop path is fine (verified with a standalone `nixie-sat`
...
//!   An initially-suspected "stale watch" theory in `nixie_sat::Solver::pop`
//!   was disproved: a standalone replay of the same clause/unit sequence
//!   through the public `nixie-sat` API answers correctly, and the memo fix
