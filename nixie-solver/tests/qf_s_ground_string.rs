...
// is either `sat` or `unsat`, never `unknown`.  nixie could already
...
/// scalar value, so Nixie's `char`-backed strings cannot hold it.  Folding it
...
/// folds neither operator, so these are cases where Nixie decides more than Z3.
#[test]
