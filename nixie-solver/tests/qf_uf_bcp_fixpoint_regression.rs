...
//! loop.
//!
//! The inner theory loop in `nixie_sat::Solver::solve_with_theory` handles a
...
//! families): z3 answered `unsat`, nixie answered `unknown` in 7 ms-5.9 s. The
...
/// problem. z3: unsat. Before the fix nixie returned `unknown` (~7 ms) because a
