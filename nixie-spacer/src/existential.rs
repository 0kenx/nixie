...
impl ExistentialProjector {
    ...
    /// [`nixie_core::ast::traversal::get_children`] (exhaustive over
    ...
    /// [`nixie_core::TermKind`] variant (its `match` has no catch-all arm)
    ...
    fn simplify_children(kind: &nixie_core::TermKind) -> Option<Vec<TermId>> {
        use nixie_core::TermKind;
...
mod tests {
    ...
    fn simplify_ground_folds_constants_and_survives_depth() {
        ...
        assert_eq!(
            terms.get(folded).map(|t| t.kind.clone()),
            Some(nixie_core::TermKind::False),
