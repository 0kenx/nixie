...
fn test_simple_counter_invariant() {
    ...
    system.add_transition_rule(
        ...
        [nixie_spacer::chc::PredicateApp::new(inv, [x])],
...
fn test_boolean_state_machine() {
    ...
    system.add_transition_rule(
        ...
        [nixie_spacer::chc::PredicateApp::new(state, [flag])],
        transition_constraint,
        state,
        [flag_prime],
    );
...
fn test_multiple_predicates() {
    ...
    system.add_transition_rule(
        ...
        [nixie_spacer::chc::PredicateApp::new(even, [x])],
        trans1,
        odd,
        [x_prime],
    );
    ...
    system.add_transition_rule(
        ...
        [nixie_spacer::chc::PredicateApp::new(odd, [x])],
        trans2,
        even,
        [x_prime],
    );
...
fn test_witness_extraction() {
    use nixie_spacer::reach::{CexState, Counterexample};

    let mut cex = Counterexample::new();
    assert!(cex.is_empty());

    let pred = nixie_spacer::chc::PredId::new(0);
    let state = nixie_core::TermId::new(42);
...
fn test_frame_manager() {
    use nixie_spacer::frames::{FrameManager, PredicateFrames};
...
fn test_proof_builder() {
    use nixie_spacer::proof::ProofBuilder;
    ...
    let invariants: HashMap<nixie_spacer::chc::PredId, SmallVec<[nixie_core::TermId; 8]>> =
        HashMap::new();
