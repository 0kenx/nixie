# SCIRS2 Policy Compliance - nixie-theories

This document tracks compliance with SCIRS2 (SciRS2) project policies for the nixie-theories crate.

## Dependencies

The nixie-theories crate uses **standard Rust libraries** instead of SciRS2-Core:

- `nixie-core` - Core AST, error handling, and SMT-LIB parsing (sibling crate)
- `nixie-sat` - SAT solver (sibling crate)
- `nixie-math` - Mathematical utilities (sibling crate)
- `nixie-nlsat` - Nonlinear arithmetic solver (sibling crate)
...
nixie-theories is a **foundational SMT theory solver library** that provides:
...
The implementations in nixie-theories are **heavily based on** the Z3 SMT solver:
...
### Notes:
- SciRS2-Core is not used because nixie-theories is domain-specific SMT solving infrastructure
