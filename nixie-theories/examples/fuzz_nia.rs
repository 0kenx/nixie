...
//! `[-6,6]^3`, and compares against [`nixie_theories::nia_cdcl::cdcl_nia_search`].
...
//! fails the run.
//!
//! Usage: `cargo run --release -p nixie-theories --example fuzz_nia -- <seed>
//! <iterations>`; set `NIXIE_NIA_CDCL_MS` to bound each search (e.g. 120).
...
fn main() {
    ...
    for it in 0..n_iters {
        ...
        let got = nixie_theories::nia_cdcl::cdcl_nia_search(&[goal], &[goal], &mut m);
        let got_v = match got {
            Some(nixie_theories::nlsat::NlDispatchResult::Sat(_)) => Some(true),
            Some(nixie_theories::nlsat::NlDispatchResult::Unsat) => Some(false),
            None => None,
        };
        ...
        if got_v.is_none() {
            // Misses are budget artifacts (NIXIE_NIA_CDCL_MS); only wrong
    ...
    for a in -6..=6 {
        for b in -6..=6 {
            for c in -6..=6 {
                ...
                if nixie_theories::ania_ground::eval_assertions_true(&[goal], m, &env) {
                    return true;
                }
            }
        }
