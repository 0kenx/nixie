...
use nixie_sat::{LBool, Lit, Solver as SatSolver, SolverConfig as SatConfig, SolverResult, Var};
...
impl BvSolver {
    ...
    /// `nixie-sat` recorded a learned clause's asserting literal at the
    ...
    fn bool_ult(
        ...
        lhs: TermId,
        rhs: TermId,
        _manager: &nixie_core::ast::TermManager,
        signed: bool,
    ) -> Option<Var> {
        let (va, vb) = match (
            self.term_to_bv.get(&lhs).cloned(),
    ...
    /// Test-only override of the `mul_array_default` memo (the env memo is
    ...
    /// accordingly (e.g. `nixie-solver`'s model builder falls back to the
    ...
    /// opinion". Used by `nixie-solver`'s debug-only model-validity net to
