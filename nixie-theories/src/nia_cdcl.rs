...
//!
//! This module is a self-contained, soundness-safe port using nixie's own
...
/// Wall-clock budget (ms). Overridable with `NIXIE_NIA_CDCL_MS`.
const DEFAULT_DEADLINE_MS: u64 = 4_000;
/// Conflict budget (0 = unlimited). Overridable with `NIXIE_NIA_CDCL_CONFLICTS`.
...
pub fn cdcl_nia_search(
    ...
    let deadline = nixie_time::Instant::now()
        + nixie_time::Duration::from_millis(
            env_u64("NIXIE_NIA_CDCL_MS", DEFAULT_DEADLINE_MS).max(100),
        );
    let max_conflicts = env_u64("NIXIE_NIA_CDCL_CONFLICTS", DEFAULT_MAX_CONFLICTS);
...
impl<'a> Encoder<'a> {
    ...
    fn bool_sort(&self) -> nixie_core::sort::SortId {
...
impl<'a> CdclSolver<'a> {
    ...
    fn solve(
        ...
        loop {
            ...
            if nixie_time::Instant::now() >= deadline {
                return None;
            }
...
mod tests {
    ...
    /// nixie reported unsat).  Must never answer `Unsat`.
    #[test]
