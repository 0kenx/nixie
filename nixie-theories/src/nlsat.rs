...
//!
//! This module wraps the NLSAT solver (from nixie-nlsat) to provide Theory trait
...
//! - NLSAT solver: nixie-nlsat::solver::NlsatSolver
//! - Integer solver: nixie-nlsat::nia::NiaSolver
...
use nixie_nlsat::nia::{NiaConfig, NiaSolver, VarType};
use nixie_nlsat::solver::{NlsatSolver, SolverResult};
use nixie_nlsat::types::{AtomKind, BoolVar, Literal};
...
fn is_numeric_sort(manager: &TermManager, sort: nixie_core::sort::SortId) -> bool {
    sort == manager.sorts.int_sort || sort == manager.sorts.real_sort
}

fn is_array_sort_id(manager: &TermManager, sort: nixie_core::sort::SortId) -> bool {
    manager
        .sorts
        .get(sort)
        .is_some_and(|s| matches!(s.kind, nixie_core::sort::SortKind::Array { .. }))
}
...
/// Env-gated dispatch tracing (`NIXIE_NIA_TRACE=1`): prints the stage that
...
macro_rules! nl_trace {
    ($($arg:tt)*) => {
        if cfg!(feature = "std") && std::env::var("NIXIE_NIA_TRACE").is_ok() {
...
pub fn dispatch_nia_constraints(
    ...
    // abstraction.  Opt-in via `NIXIE_NIA_CDCL`: with learning enabled the
    ...
    #[cfg(feature = "std")]
    if std::env::var("NIXIE_NIA_CDCL").is_ok()
        && std::env::var("NIXIE_NIA_CDCL")
            .map(|v| v != "0")
            .unwrap_or(false)
    ...
    // goals, so it runs by default only under a size ceiling; `NIXIE_NIA_CNF`
    // forces it on for any size.
    let cnf_enabled = n_symbols <= CNF_MAX_DEFAULT_SYMBOLS
        || (cfg!(feature = "std") && std::env::var("NIXIE_NIA_CNF").is_ok());
...
pub(crate) fn solve_conjunction_nia(
    ...
    // re-sampling in `nixie-nlsat` (bare `x*y=c` no longer collapses to Unsat).
    ...
    while let Some(id) = stack.pop() {
        ...
        match &n.kind {
            ...
            TermKind::Div(_, _) | TermKind::Mod(_, _) | TermKind::Select(_, _) => {
                ...
                let children = nixie_core::ast::traversal::get_children(&n.kind);
...
/// (still reachable with `NIXIE_NIA_CNF`).
...
fn augment_select_values(
    ...
    for &a in assertions {
        for st in nixie_core::ast::collect_subterms(a, manager) {
...
fn root_obj_coefficients(
    poly: &nixie_math::polynomial::Polynomial,
