...
use nixie_core::error::{NixieError, Result};
...
impl ReplaceSolver {
    ...
    fn propagate_constraint(&mut self, constraint: &ReplaceConstraint) -> Result<()> {
        ...
        if let Some(existing) = self.assignments.get(&constraint.result) {
            if existing != &result_val {
                ...
                return Err(NixieError::Internal("replace result conflict".to_string()));
            }
        } else {
            // Deduce result value
            self.deductions
    ...
    pub fn check(&self) -> Result<()> {
        if self.conflict.is_some() {
            return Err(NixieError::Internal(
...
impl ReplaceBuilder {
    ...
    pub fn build(self, result: TermId, origin: TermId) -> Result<ReplaceConstraint> {
        let source = self
            .source
            .ok_or_else(|| NixieError::Internal("missing source".to_string()))?;
        let pattern = self
            .pattern
            .ok_or_else(|| NixieError::Internal("missing pattern".to_string()))?;
        let replacement = self
            .replacement
            .ok_or_else(|| NixieError::Internal("missing replacement".to_string()))?;
