...
use nixie_theories::euf::ProofStep;
use nixie_theories::euf::{EMatchEngine, Pattern, QuantifiedFormula, Trigger, VarId};
use nixie_theories::nlsat::{TermPolyTranslator, term_is_nonlinear};
use nixie_theories::set::{SetExpr, SetSolver, SetSort};
use nixie_theories::string::sequence::{IntExpr, SeqExpr, SeqRewriter};
use nixie_theories::string::{Regex, RegexOp};
...
fn deep_set_expr_constraint_extraction_returns() {
    ...
    let ok = on_worker_stack(|| {
        ...
        solver
            .add_constraint(nixie_theories::set::SetConstraint::Member {
                element: 1,
                set: expr,
                sign: true,
            })
