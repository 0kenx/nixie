...
use nixie_theories::{Theory, TheoryCheckResult as TheoryResult};
...
/// here; this test documents it for whichever wave owns nixie-sat /
/// nixie-theories check() plumbing):
///
...
///
/// - `nixie_sat::Solver::add_clause` special-cases unit (single-literal)
///   clauses by pushing them straight onto the trail via
///   `trail.assign_decision(lit)` with **no backing clause** (see
///   `nixie-sat/src/solver/mod.rs`, the `len() == 1` arm). Both
...
/// - `FpSolver::check()` (`nixie-theories/src/fp/solver/mod.rs`, the `if
...
///
/// This is a soundness bug spanning `nixie-sat`'s unit-clause fast path
/// and/or `nixie-theories`'s blanket double-solve retry, not an error in
/// `assert_is_normal`'s constraint logic. Fixing it requires either making
/// `nixie_sat::Solver::add_clause`'s unit-clause fast path re-validate
...
#[ignore = "confirmed production bug: FpSolver::check() double-solve retry \
            (nixie-theories/src/fp/solver/mod.rs) turns a genuine Unsat \
            (assert_const subnormal + assert_is_normal) into a spurious \
            Sat; root cause traced into nixie_sat::Solver::add_clause's \
