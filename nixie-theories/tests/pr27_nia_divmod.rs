...
//! dispatcher (`nixie_theories::nlsat::dispatch_nia_constraints` /
...
//! `div`/`mod` problem is deliberately left to `nixie-solver`'s
...
//! `nixie-solver/tests/pr27_divmod_semantics.rs` – this file only checks that
...
use nixie_theories::nlsat::{NlDispatchResult, dispatch_nia_constraints};
...
fn test_pr27_nia_divmod_negative_divisor_unreachable_remainder_is_unsat() {
    ...
    // `nixie-theories/src/nlsat.rs` and the deferred bounded-enumeration
...
/// `nixie-solver`'s `arith_axioms::int_constant` on a *folded* (not bare
...
/// `IntConst` (see `nixie-core/src/smtlib/parser/terms.rs`'s numeral leaf),
