...
function getConfig<T>(key: string, defaultValue: T): T {
  const config = vscode.workspace.getConfiguration("nixie");
...
/**
 * Find nixie executable path
 */
function findNixiePath(): string {
  const configPath = getConfig<string>("executablePath", "nixie");
  ...
  // name while at it.
  const binaryName = process.platform === "win32" ? "nixie.exe" : "nixie";
...
async function startLspClient(context: vscode.ExtensionContext): Promise<void> {
  ...
  try {
    await client.start();
    outputChannel.appendLine("Nixie Language Server started");
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    outputChannel.appendLine(`Failed to start LSP server: ${errorMessage}`);
    vscode.window.showWarningMessage(
      `Nixie LSP server failed to start: ${errorMessage}. Falling back to basic mode.`
    );
    client = undefined;
  }
}
...
async function runSolver(document: vscode.TextDocument): Promise<void> {
  ...
  outputChannel.appendLine(`Running Nixie solver on: ${document.fileName}`);
  ...
  try {
    const result = await new Promise<string>((resolve, reject) => {
      const process = cp.spawn(nixiePath, args);
      let stdout = "";
      let stderr = "";
    ...
    if (result.includes("sat") && !result.includes("unsat")) {
      vscode.window.showInformationMessage("Nixie: SAT (Satisfiable)");
    } else if (result.includes("unsat")) {
      vscode.window.showInformationMessage("Nixie: UNSAT (Unsatisfiable)");
    } else if (result.includes("unknown")) {
      vscode.window.showWarningMessage("Nixie: UNKNOWN");
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    outputChannel.appendLine(`Error: ${errorMessage}`);
    vscode.window.showErrorMessage(`Nixie solver error: ${errorMessage}`);
  }
}
...
export async function activate(
  ...
  outputChannel = vscode.window.createOutputChannel("Nixie SMT Solver");
  diagnosticsCollection =
    vscode.languages.createDiagnosticCollection("smtlib2");

  outputChannel.appendLine("Nixie SMT Solver extension activated");
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("nixie.runSolver", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("nixie.checkSat", async () => {
      const editor = vscode.window.activeTextEditor;
      if (editor && editor.document.languageId === "smtlib2") {
        await runSolver(editor.document);
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("nixie.getModel", async () => {
      const editor = vscode.window.activeTextEditor;
      if (editor && editor.document.languageId === "smtlib2") {
        await runSolver(editor.document);
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("nixie.restartLanguageServer", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("nixie.showSolverInfo", async () => {
      const nixiePath = findNixiePath();
      try {
        const result = cp.execSync(`${nixiePath} --version`).toString();
        vscode.window.showInformationMessage(`Nixie Solver: ${result.trim()}`);
      } catch {
        vscode.window.showErrorMessage(
          "Could not get Nixie solver information. Is it installed?"
        );
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration(async (event) => {
      if (event.affectsConfiguration("nixie.enableLsp")) {
        const lspEnabled = getConfig<boolean>("enableLsp", true);
