# Browser Testing Guide for Nixie WASM

This guide provides instructions for testing Nixie WASM across different browser environments.
...
2. **Check WASM size**:
   ```bash
   ls -lh pkg/*.wasm
   wasm-opt --print-size pkg/nixie_wasm_bg.wasm
   ```
