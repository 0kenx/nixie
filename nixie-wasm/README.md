# Nixie WASM - WebAssembly Bindings for Nixie SMT Solver

WebAssembly bindings for the Nixie SMT solver, enabling high-performance SMT solving directly in web browsers and Node.js.
...
| Version | 0.3.1 |
| Status | Stable |
| Tests | 155 passing (`cargo nextest run -p nixie-wasm --all-features`) |
| Source files | 30 |
| Public API items | 471 |

## Overview

This crate provides comprehensive JavaScript/TypeScript bindings for running Nixie in web browsers and Node.js environments. It features a complete API with async support, Web Worker compatibility, and structured error handling.
...
```javascript
import init, { WasmSolver, version } from './pkg/nixie_wasm.js';
...
```typescript
import init, { WasmSolver, version, type Model, type SatResult } from './pkg/nixie_wasm';
...
The package includes comprehensive TypeScript declarations (`nixie-wasm.d.ts`) providing:
...
```javascript
const { WasmSolver, version } = require('./pkg/nixie_wasm');
...
- [nixie-core](../nixie-core) - Core SMT solver implementation
- [nixie-solver](../nixie-solver) - High-level solver interface
- [nixie-cli](../nixie-cli) - Command-line interface
