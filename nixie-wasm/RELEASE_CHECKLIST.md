# Nixie WASM Release Checklist

This document provides a step-by-step checklist for releasing @cooljapan/nixie to NPM and deploying the playground.

## Pre-Release Checklist

### Code Quality
- [x] All tests passing (`cargo test`)
- [x] No compiler warnings in nixie-wasm
...
- [ ] Final WASM size is acceptable (target <2MB)
  ```bash
  ls -lh pkg-web/nixie_wasm_bg.wasm
  ```
...
- [ ] Verify unpkg
  ```
  https://unpkg.com/@cooljapan/nixie@X.Y.Z/
  ```

- [ ] Verify jsDelivr
  ```
  https://cdn.jsdelivr.net/npm/@cooljapan/nixie@X.Y.Z/
  ```
...
- [ ] Test CDN loading
  ...
  ```html
  <script type="module">
    import init from 'https://unpkg.com/@cooljapan/nixie@X.Y.Z/pkg/nixie_wasm.js';
    await init();
...
5. [ ] Verify deployment
   ```
   https://your-org.github.io/nixie/
   ```
...
```bash
npm deprecate @cooljapan/nixie@X.Y.Z "This version has issues, use X.Y.Z+1 instead"
```

### Option 2: Unpublish (within 72 hours)
```bash
npm unpublish @cooljapan/nixie@X.Y.Z
```
...
- [ ] Check NPM download stats
  ```
  https://npm-stat.com/charts.html?package=@cooljapan/nixie
  ```
...
- [ ] Check CDN hit rates
  ```
  https://www.jsdelivr.com/package/npm/@cooljapan/nixie
  ```
