# SCIRS2 POLICY - nixie-wasm

## Purpose

This document defines the policy for using SciRS2 libraries in the nixie-wasm crate.

## Policy Statement

The nixie-wasm crate is a WebAssembly binding layer for the Nixie SMT solver. As such, it primarily interfaces with JavaScript and does NOT directly use numerical or scientific computing libraries.
...
1. It is a thin binding layer over nixie-core and nixie-solver
2. All numerical and scientific computation is handled by the core nixie libraries
...
If nixie-wasm ever requires numerical computing capabilities (which it currently does not), the following approach must be taken:

1. **First choice**: Delegate to nixie-core or nixie-solver which already use SciRS2-Core
...
1. Evaluate whether the feature belongs in nixie-core instead
