...
//!
//! This benchmark suite measures and compares bundle sizes between Nixie-WASM
...
/// Compare Nixie vs Z3 bundle sizes
fn bench_nixie_vs_z3(c: &mut Criterion) {
    let mut group = c.benchmark_group("nixie_vs_z3");
    ...
    // Measure Nixie sizes
    let nixie_minimal = measure_bundle_size(&BundleConfig::minimal());
    let nixie_standard = measure_bundle_size(&BundleConfig::standard());
    let nixie_full = measure_bundle_size(&BundleConfig::full());

    // Print comparison reports
    println!("\n=== Bundle Size Comparison ===");
    z3_size.print_report(None);
    nixie_minimal.print_report(Some(&z3_size));
    nixie_standard.print_report(Some(&z3_size));
    nixie_full.print_report(Some(&z3_size));
    ...
    group.bench_function("nixie_minimal_load", |b| {
        b.iter(|| {
            // Simulate Nixie minimal load time (~50ms)
            std::thread::sleep(std::time::Duration::from_millis(1));
            black_box(nixie_minimal.clone())
        });
    });

    group.bench_function("nixie_standard_load", |b| {
        b.iter(|| {
            // Simulate Nixie standard load time (~150ms)
            std::thread::sleep(std::time::Duration::from_millis(3));
            black_box(nixie_standard.clone())
        });
    });
...
mod tests {
    ...
    fn test_z3_comparison() {
        ...
        let nixie_gzip = measure_bundle_size(&BundleConfig::full()).gzip;

        // Nixie should be at least 60% smaller than Z3
        let improvement = ((z3_gzip - nixie_gzip) as f64 / z3_gzip as f64) * 100.0;
        assert!(
            improvement >= 60.0,
