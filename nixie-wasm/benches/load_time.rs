...
//! for Nixie-WASM, comparing different configurations and optimization levels.
//!
//! # Metrics
//!
...
/// Benchmark Nixie vs Z3 load times
fn bench_nixie_vs_z3_load(c: &mut Criterion) {
    let mut group = c.benchmark_group("nixie_vs_z3_load");
    ...
    // Nixie load times
    for config in LoadConfig::all() {
        let name = format!("nixie_{}", config.name);
...
mod tests {
    ...
    fn test_z3_comparison() {
        let z3_load = 500.0;
        let nixie_load = measure_load_time(&LoadConfig::full()).total_ms;

        // Nixie should be at least 2x faster
        let speedup = z3_load / nixie_load;
        assert!(
            speedup >= 2.0,
