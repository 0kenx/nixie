# Nixie WASM API Reference

Complete reference documentation for the Nixie WASM API.
...
The main solver class for interacting with Nixie from JavaScript/TypeScript.
...
**Example:**
```javascript
const name = solver.getInfo("name"); // "Nixie"
...
Returns the Nixie WASM version.

**Returns:** `string` - Version in semver format

**Example:**
```javascript
import { version } from '@cooljapan/nixie';
...
Nixie WASM includes full TypeScript declarations. Import types as:

```typescript
import { WasmSolver, WasmErrorKind } from '@cooljapan/nixie';
