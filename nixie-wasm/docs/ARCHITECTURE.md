# Nixie WASM Architecture

This document describes the architecture and design of the Nixie WASM bindings.
...
Nixie WASM provides WebAssembly bindings for the Nixie SMT solver, enabling SMT solving directly in web browsers and Node.js environments. The architecture is designed to:
...
The main entry point for users. Wraps `nixie_solver::Context` and provides:
...
wasm-opt -Oz -o pkg/nixie_wasm_bg.wasm pkg/nixie_wasm_bg.wasm

# 3. Compress for distribution
gzip -k pkg/nixie_wasm_bg.wasm
brotli pkg/nixie_wasm_bg.wasm
```
...
nixie-wasm/
├── package.json
├── nixie_wasm.js         # JavaScript glue code
├── nixie_wasm.d.ts       # TypeScript declarations
├── nixie_wasm_bg.wasm    # WASM binary
└── README.md
```
...
```html
<script type="module">
  import init, { WasmSolver } from 'https://cdn.jsdelivr.net/npm/nixie-wasm/nixie_wasm.js';
...
- [Nixie Core Documentation](../../nixie-core/README.md)
