...
This guide provides strategies for optimizing the performance of Nixie WASM in your applications.
...
**worker.js:**
```javascript
import init, { WasmSolver } from './nixie-wasm/nixie_wasm.js';
...
async function getSolver() {
  if (!wasmModule) {
    wasmModule = await import('./nixie-wasm/nixie_wasm.js');
