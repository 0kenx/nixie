...
# Nixie WASM NPM Publishing Script
...
if [ ! -f "Cargo.toml" ] || [ ! -f "package.json" ]; then
    print_error "Must be run from nixie-wasm directory"
    exit 1
fi
...
if npm view @cooljapan/nixie@$CARGO_VERSION version &> /dev/null; then
...
if command -v wasm-opt &> /dev/null; then
    wasm-opt -Oz --enable-bulk-memory pkg-web/nixie_wasm_bg.wasm -o pkg-web/nixie_wasm_bg.wasm
...
for dir in pkg-web pkg-nodejs pkg-bundler; do
    if [ -d "$dir" ]; then
        WASM_SIZE=$(stat -f%z "$dir/nixie_wasm_bg.wasm" 2>/dev/null || stat -c%s "$dir/nixie_wasm_bg.wasm" 2>/dev/null)
        SIZE_MB=$(echo "scale=2; $WASM_SIZE / 1024 / 1024" | bc)
        echo "  $dir: ${SIZE_MB} MB"
    fi
done

# Patch package name for @cooljapan/nixie scope (wasm-pack generates "nixie-wasm")
for dir in pkg-web pkg-nodejs pkg-bundler; do
    if [ -f "$dir/package.json" ]; then
        sed -i '' 's/"name": "nixie-wasm"/"name": "@cooljapan\/nixie"/' "$dir/package.json"
    fi
done
...
# Confirm publication
echo ""
print_warn "About to publish @cooljapan/nixie@$CARGO_VERSION to NPM"
...
print_info "Successfully published @cooljapan/nixie@$CARGO_VERSION"
...
echo "  unpkg: https://unpkg.com/@cooljapan/nixie@$CARGO_VERSION/"
echo "  jsdelivr: https://cdn.jsdelivr.net/npm/@cooljapan/nixie@$CARGO_VERSION/"
