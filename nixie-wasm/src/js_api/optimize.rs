...
//!
//! `Command` (`nixie_core::smtlib::Command`) has no `Minimize`/`Maximize`/
//! `AssertSoft` variant, so those SMT-LIB extension commands cannot be routed
//! through [`nixie_solver::Context::execute_script`] at all -- an unrecognized
...
//!
//! The real fix wires these through [`nixie_solver::Optimizer`] (a genuine
//! lexicographic-objective / lower-bound-search optimizer that already lives
//! in `nixie-solver`, already a dependency of this crate) instead. That
...
//! `nixie_solver::Context`, except this package's owned files are only
//! `nixie-wasm/src/js_api/*` and `nixie-wasm/tests/*`. `Context` already
...
//! as `produce-proofs`); this reuses it as a namespaced (`__nixie_wasm_*`
...
//! `Bool`-sorted placeholder. `nixie_core::smtlib::Parser::with_context` (the
//! "parser seeding" API) exists for exactly this, but `Parser` itself is not
//! re-exported from `nixie_core::smtlib` (only `Command`/`parse_script`/
...
//! the *public* [`nixie_core::smtlib::parse_script`] in one call: since that
...
use nixie_core::smtlib::{Command, Printer, parse_script, parse_term};
use wasm_bindgen::prelude::*;

/// Newline-joined list of every symbol name ever declared through
/// `declareConst`/`declareFun` (0-ary), keyed into [`nixie_solver::Context`]'s
/// generic option store (see the module-level docs above).
const DECL_NAMES_KEY: &str = "__nixie_wasm_decl_names";
...
impl WasmSolver {
    ...
    /// wired to the real `nixie_solver::Optimizer` – they are never silently
    /// dropped.
    ///
    /// # Parameters
    ///
    ...
    /// via [`nixie_solver::Optimizer`], a real lexicographic-objective
    ...
    /// convention `nixie_solver::Optimizer::optimize` itself uses.
    ///
    /// # A note on `getMinimalModel()`/`getModel()` after `optimize()`
    ///
    /// `Optimizer` solves against its own freshly-built internal solver (see
    /// `nixie_solver::optimization::Optimizer::build_solver`), never touching
    ...
    pub fn optimize(&mut self) -> Result<JsValue, JsValue> {
        ...
        let mut opt = nixie_solver::Optimizer::new();
        ...
        if !soft_terms.is_empty() {
            ...
            for (idx, (term, weight)) in soft_terms.iter().enumerate() {
                ...
                let relax_var = self
                    .ctx
                    .terms
                    .mk_var(&format!("__nixie_wasm_soft_relax_{idx}"), bool_sort);
        ...
        match opt_result {
            nixie_solver::OptimizationResult::Optimal { value, model } => {
    ...
    /// [`nixie_solver::Optimizer`] -- see the `optimize()` docs for the exact
    /// construction.
    ///
    /// # Parameters
    ///
    ...
    /// (`nixie_core::ast::interpolation::InterpolationContext`) over the
    /// structured proof DAG produced by the check. `nixie_solver::Context`
    ...
    pub fn compute_interpolant(
        ...
        Err(WasmError::new(
            ...
            "Craig interpolation is not yet available from the WASM bindings: the \
             ...
             run Pudlák interpolation (nixie_core::ast::interpolation). Returning an \
    ...
    ///
    /// This calls `nixie_core::qe::QeLiteSolver`, a real (not fabricated)
    ...
    /// require a fix inside `nixie-core` (out of scope for this binding).
    ...
    pub fn eliminate_quantifiers(&mut self, formula: &str) -> Result<String, JsValue> {
        ...
        let mut qe = nixie_core::qe::QeLiteSolver::new();
        let qe_result = qe.eliminate(term, &mut self.ctx.terms);

        let result_term = match qe_result {
            nixie_core::qe::QeLiteResult::Eliminated(t) => t,
            nixie_core::qe::QeLiteResult::Simplified(t) => t,
            nixie_core::qe::QeLiteResult::Unchanged => {
                return Err(WasmError::new(
                    WasmErrorKind::NotSupported,
            ...
            nixie_core::qe::QeLiteResult::Error(msg) => {
                return Err(WasmError::new(
                    WasmErrorKind::Unknown,
...
impl WasmSolver {
    ...
    /// `Solver` (see `nixie_solver::optimization::Optimizer::build_solver`),
...
/// be exercised here at all -- see `nixie-wasm/tests/audit_wasm_p2.rs` for
