//! Promise-based Async API for Nixie
//!
//! This module provides a Promise-based async API for Nixie solver operations,
...
pub struct AsyncSolver {
    ...
    ctx: Rc<RefCell<nixie_solver::Context>>,
    /// Cancellation flag
...
impl AsyncSolver {
    ...
    pub fn new() -> Self {
        Self {
            ctx: Rc::new(RefCell::new(nixie_solver::Context::new())),
    ...
    pub fn reset_async(&self) -> Promise {
        ...
        future_to_promise(async move {
            *ctx.borrow_mut() = nixie_solver::Context::new();
            Ok(JsValue::UNDEFINED)
        })
    }

    // Helper to parse sorts - returns SortId from context's SortManager
    fn parse_sort(&self, sort_name: &str) -> Result<nixie_core::SortId, JsValue> {
        match sort_name {
