...
mod tests {
    ...
    // `execute_script` call gets a brand-new `nixie_core` parser whose
    // declared-symbol table starts empty, so a symbol declared outside
    // that same script/parse is reported as "unknown constant or
    // symbol". This is a pre-existing defect in `nixie-core`'s
    // parser/`nixie-solver::Context::execute_script` (outside this
