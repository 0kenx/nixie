...
//!
//! This module provides utilities for running Nixie solver operations
...
//!
//! ```javascript
//! import { WorkerHandler } from '@cooljapan/nixie/worker';
//!
...
///
/// Nixie WASM does not enable the `atomics`/`bulk-memory` wasm target
...
impl WorkerHandler {
    ...
    /// checked inside `check_sat` itself: `nixie_solver::Context::
    /// check_sat` has no cancellation hook to poll (adding one is out of
    /// scope for `nixie-wasm`), so once a `check_sat` call starts it always
