...
//!
//! ```ignore
//! use nixie_wasm::optimize::symbol_stripping::{SymbolStripper, StripConfig};
//!
...
mod tests {
    ...
    fn test_keep_patterns() {
        ...
        stripper.register_symbol(SymbolInfo {
            name: "nixie_internal_func".to_string(),
            symbol_type: SymbolType::Function,
            size_bytes: 50,
            exported: false,
        });

        let config = StripConfig::aggressive().keep_name("nixie_");
        let result = stripper.strip(&config);
