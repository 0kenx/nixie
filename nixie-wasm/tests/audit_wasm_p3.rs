...
//! - `nixie-wasm/src/js_api/model.rs` (`getUnsatCore` returned every
//!   assertion instead of the real unsat core)
//! - `nixie-wasm/src/js_api/solver_core.rs` (`executeAsync`/
//!   `executeWithProgress` split scripts at fixed 20-line boundaries,
//!   breaking multi-line commands)
//! - `nixie-wasm/src/js_api/streaming.rs` (`StreamingSolver.nextModelEntry`
//!   always returned `None`; `startModelStream` returned a disconnected
//!   controller)
//! - `nixie-wasm/src/js_api/worker_support.rs` (`WorkerPool` never executed
...
//! brand-new `nixie_core` parser whose declared-symbol table starts empty,
...
//! reproduces with plain `nixie_solver::Context` alone, independent of
//! WASM/JS at all) -- a pre-existing limitation in `nixie-core`'s parser /
//! `nixie-solver::Context::execute_script`, outside this package's owned
//! files (`nixie-wasm/src/js_api/*`, `lazy_loader.rs`, `package.json`).
...
// `DataChunk`, etc. live one module deeper (`nixie_wasm::js_api::*`, only
...
/// named unsat core.
///
/// Note: `nixie-solver`'s core builder is a conservative (non-minimal)
/// "all named assertions" implementation (see
/// `nixie-solver/src/solver/model_builder.rs::build_unsat_core`), so this
