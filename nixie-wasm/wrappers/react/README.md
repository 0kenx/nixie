...
React wrapper for Nixie WASM SMT Solver.

## Installation

```bash
npm install @nixie/react @cooljapan/nixie
```
...
```tsx
import { useSolver } from '@nixie/react';
...
```tsx
import { SolverProvider, useSolverContext } from '@nixie/react';
...
```tsx
import { useSolverWorker } from '@nixie/react';
