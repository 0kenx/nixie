/**
 * Svelte store for Nixie SMT Solver
 */
...
/**
 * Create a Svelte store for the Nixie SMT Solver
 *
 * @example
 * ```svelte
 * <script>
 *   import { createSolverStore } from '@nixie/svelte';
 *
...
export function createSolverStore(options: SolverOptions = {}): SolverStore {
  ...
  async function init() {
    try {
      ...
      const { default: initWasm, WasmSolver } = await import('@cooljapan/nixie');
