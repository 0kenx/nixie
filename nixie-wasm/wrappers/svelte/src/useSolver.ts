/**
 * Svelte composable-style function for using Nixie SMT Solver
 */
...
/**
 * Function for using the Nixie SMT Solver in Svelte components
 *
 * @example
 * ```svelte
 * <script>
 *   import { useSolver } from '@nixie/svelte';
...
export function useSolver(options: SolverOptions = {}): UseSolverResult {
  ...
  onMount(async () => {
    try {
      ...
      const { default: init, WasmSolver } = await import('@cooljapan/nixie');
