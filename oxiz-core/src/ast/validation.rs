            ...
            EvalFrame::Combine(id) => {
                ...
                let value = match &term.kind {
                    ...
                    TermKind::BvNot(arg) => match operand(arg, cache) {
                        ...
                        Some(ModelValue::BitVec { value, width }) => bitvec_width_mask(width)
                            .map(|mask| ModelValue::BitVec {
...
fn bv_binary(
    ...
    match op {
        ...
        ExactBvOp::Slt => Some(ModelValue::Bool(
            signed_cmp() == core::cmp::Ordering::Less,
        )),
