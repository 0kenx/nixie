...
fn main() {
    ...
    println!(
        ...
        r, el.as_secs_f64(), s.num_vars(),
        st.conflicts, st.decisions, st.propagations, st.restarts,
        st.learned_clauses, st.unit_clauses, st.binary_clauses,
        st.deleted_clauses, st.learned_clauses.saturating_sub(st.deleted_clauses),
        st.total_lbd, st.avg_lbd(), st.minimizations, st.literals_removed,
        st.chrono_backtracks, st.non_chrono_backtracks,
    );
}
