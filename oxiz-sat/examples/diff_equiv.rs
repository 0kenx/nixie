...
fn rand(seed: &mut u64) -> u64 { *seed^=*seed<<13; *seed^=*seed>>7; *seed^=*seed<<17; *seed }
fn to_cnf(nvars: usize, clauses: &[Vec<i32>]) -> String {
    let mut s = format!("p cnf {nvars} {}\n", clauses.len());
    for c in clauses { for &v in c { s.push_str(&format!("{v} ")); } s.push_str("0\n"); }
    s
}
fn gen_formula(seed: &mut u64, nvars: usize, nclauses: usize) -> Vec<Vec<i32>> {
    ...
    for _ in 0..nclauses {
        ...
        while c.len() < len {
            ...
            if !c.contains(&lit) && !c.contains(&-lit) { c.push(lit); }
        }
        out.push(c);
    }
    out
...
fn solve(prep: bool, cnf: &str) -> (oxiz_sat::SolverResult, Vec<u8>) {
    ...
    let r = s.solve();
    let model: Vec<u8> = (0..s.num_vars()).map(|i| match s.model().get(i) { Some(LBool::True)=>1u8, Some(LBool::False)=>0u8, _=>2u8 }).collect();
    (r, model)
}
fn model_ok(model: &[u8], clauses: &[Vec<i32>]) -> bool {
    clauses.iter().all(|c| c.iter().any(|&v| { let vi=v.unsigned_abs() as usize-1; model.get(vi).copied()==Some(if v>0 {1} else {0}) }))
}
fn main() {
    ...
    for i in 0..iters {
        ...
        let agree = matches!((r_off,r_on),(oxiz_sat::SolverResult::Sat,oxiz_sat::SolverResult::Sat)|(oxiz_sat::SolverResult::Unsat,oxiz_sat::SolverResult::Unsat));
        if !agree { mismatch+=1; if mismatch<=3 { eprintln!("MISMATCH iter={i} nv={nvars} nc={nclauses}: off={r_off:?} on={r_on:?}"); } continue; }
        if matches!(r_on, oxiz_sat::SolverResult::Sat) { sat+=1; if !model_ok(&m_on,&f) { invalid+=1; if invalid<=3 { eprintln!("INVALID MODEL iter={i} nv={nvars} nc={nclauses}"); } } }
    }
    println!("iters={iters} sat={sat} mismatches={mismatch} invalid_models={invalid}");
    if mismatch>0||invalid>0 { std::process::exit(1); }
}
